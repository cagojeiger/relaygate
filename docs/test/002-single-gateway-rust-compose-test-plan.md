...
`v02_public_sdk` integration target가 다음 local 경로를 검증합니다.
...
- 같은 RouteAddress를 여러 Relay가 publish하고 dial마다 하나만 선택
- 같은 Relay의 동일 RouteAddress 중복 listen 거절
- self Binding만 존재하면 실패
- valid publish/dial grant 허용, invalid JWT·action/address mismatch 거절
