...
| `T-SDK-10` | `SDK-017` | credential 거절이나 권한 폐기는 영향받은 handle만 `BLOCKED`로 만들고 자동 재등록·동일 handle 재활성화를 하지 않는다. 폐기 전에 admission을 마친 queued·accepted Pipe와 sibling handle, shared session은 유지하고 신규 Pipe만 중단한다. 기존 handle을 닫은 뒤 새 credential의 새 `listen`은 새 binding incarnation으로 성공할 수 있다. |
| `T-SDK-11` | `SDK-019` | 같은 runtime에서 동일 ClientId의 pending listen과 handle 생성을 경쟁시키면 reservation 또는 handle 하나만 남고 나머지는 `ALREADY_EXISTS`다. attempt 실패나 handle close 뒤 새 listen은 새 binding incarnation으로 성공한다. |
| `T-SDK-12` | `SDK-023` | commit된 connect의 terminal 응답을 blackhole 처리한다. deadline 뒤 호출은 `MAYBE_OBSERVED`로 한 번만 끝나고 current ConnectorSession의 다른 attempt·Pipe도 종료되며, 새 session은 새 identity를 쓰고 이전 OPEN을 replay하지 않는다. |
...
| `T-REG-04` | `REG-009`, `REG-014` | binding 제거와 권한 폐기는 local index에서 즉시 제외되고 새 snapshot에서 빠진다. 해당 key의 마지막 binding이면 빈 `Update`가 아니라 `Deregister`하며, 다른 binding과 sibling handle은 유지된다. |
...
| `T-RT-02` | `RT-010`, `RT-011`, `RT-012`, `RT-013`, `RT-014`, `RT-015`, `RT-016`, `RT-017` | `Register`가 mapping 없는 RT 발급 lease를 만들고 duplicate Register가 state를 바꾸지 않는지 확인한다. active lease의 full snapshot atomic replace, equal revision idempotency, conflicting·lower revision과 stale LeaseId 거절 및 다른 registration 보존도 검증한다. |
...
| `T-STATE-CONNECTOR` | `STATE-CS-001`, `STATE-CS-002` | Connector transport마다 새 session identity를 만들고 단절·bounded write failure·commit된 OPEN terminal-response deadline 시 그 session의 attempt·Pipe·RelayStream만 terminal cleanup되는지 확인한다. |
| `T-STATE-LISTENER` | `STATE-LA-001`, `STATE-LA-002`, `STATE-LA-003`, `STATE-LA-004`, `STATE-LA-005`, `STATE-LA-006`, `STATE-LH-001`, `STATE-LH-002`, `STATE-LH-003`, `STATE-LH-004`, `STATE-LH-005`, `STATE-LH-006`, `STATE-LH-007`, `STATE-LS-001`, `STATE-LS-002` | duplicate create는 state를 만들지 않는다. 최초 attempt는 commit 전 대기만 허용하고 commit 뒤 실패·deadline·session loss에는 terminal 실패한다. 성공해 반환된 Listener만 suspend/re-register하며 credential failure에서는 BLOCKED가 되고 같은 handle은 부활하지 않는다. transport·bounded write failure, REGISTER response deadline 또는 Gateway OFFER deadline은 selected ListenerSession 전체만 닫고 새 identity로 재연결한다. |
...
| `T-EDGE-06` | RT restart -> READY-empty -> Gateway 새 lease Register -> current snapshot Update | 기존 Pipe와 local OPEN은 유지되고 remote connect는 수렴 중 NOT_FOUND일 수 있으며 Update 뒤 복구된다. |
...
| `T-EDGE-14` | 같은 PeerTransport를 공유하는 ConnectorSession S1·S2 중 S1 단절 | S1의 pending attempt, Pipe와 RelayStream만 닫히며 S2, shared PeerTransport, Listener binding과 RT mapping은 유지된다. |
...
| `T-EDGE-28` | 반환된 Listener A의 recovery REGISTERED 응답과 SDK의 old ListenerSession 종료 신호가 silent partition에서 유실 -> SDK는 L2에 A를 새 identity로 등록 -> Gateway는 old L1과 new L2 binding을 잠시 함께 보유 -> L1에 OPEN/OFFER 무응답 | N:M 때문에 두 binding 공존은 허용되지만 L1은 성공을 만들지 못한다. L1 OFFER deadline이 L1의 binding·Pipe를 전부 제거하고 L2는 유지되며 같은 connect는 reroute하지 않는다. |
