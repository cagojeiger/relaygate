...
| `T-ERR-06` | `ERR-013`, `ERR-014`, `ERR-015` | queue 미도달 증명, OPENED 확인과 둘 다 불가능한 terminal failure를 구분한다. `MAYBE_OBSERVED`는 실제 queue 여부를 단정하지 않고 replay하지 않는다. unknown ConnectorSession/Connection/Stream identity의 terminal frame은 live state를 만들지 않고 current identity의 non-owner frame은 target을 변경하지 않는다. conforming Gateway는 OFFER를 재전송하지 않고 Listener SDK는 종료 PipeId tombstone을 보관하지 않는다. |
