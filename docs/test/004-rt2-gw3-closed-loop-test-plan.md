...
일회성 certificate/Secret을 사용하며 종료 시 해당 cluster만 삭제합니다.

## acceptance

| ID | 시나리오 | 통과 조건 |
| --- | --- | --- |
| `KIND-01` | TLS/admission | 올바른 CA/name/token만 연결되고 wrong name/token은 state 없이 실패 |
...
모든 ID에 command output, pod state, metric snapshot과 로그 검색 결과가 있어야 합니다. Helm render 성공,
pod Ready 또는 단일 echo만으로 완료하지 않습니다.
