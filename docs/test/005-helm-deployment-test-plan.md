...
| automatic reload | edge는 GW만, internal은 role별 leaf·공개 trust Secret을 StatefulSet metadata에서 watch |
| reload isolation | edge/internal 옵션 독립, watch 활성화만으로 Pod template 변경 없음, edge 갱신 시 RT 유지 |
...
| plaintext + internal certManager/autoReload | template rejection; edge autoReload는 허용 |
...
`.github/scripts/helm_transport_test.py`는 두 모드와 edge/internal watch 조합·위치·중복 제거를 검증한다. Kind CI는
