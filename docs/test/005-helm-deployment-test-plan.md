...
RouteTable         = logical shard별 Service 1개 + desired process 1개
...
| `AC-HELM-03` | RT shard마다 stable ClusterIP Service와 `replicas=1`, `strategy=Recreate` Deployment가 정확히 하나씩 존재한다. |
| `AC-HELM-04` | immutable ConfigMap은 ordered `rt-0..rt-(M-1)`와 각 stable Service FQDN을 한 exact JSON artifact로 만들고 모든 process가 같은 directory를 read-only mount한다. directory checksum은 Gateway와 RT pod template에 같고, ordinary Helm upgrade로 artifact 변경을 허용하지 않는다. |
| `AC-HELM-05` | chart는 Secret data, SDK workload, PVC, `emptyDir`, Ingress, HPA와 RT replica를 만들지 않는다. credential은 existing Secret key reference로만 전달한다. |
...
| `AC-HELM-08` | schema는 replica/shard 수, port, image, log format, Secret reference의 잘못된 값을 render 전에 거절하고 extraEnv가 chart-managed 변수와 충돌하면 실패한다. |
...
CI는 render 결과의 resource 수, identity/locator env, ShardDirectory bytes, update strategy,
...
4. helm test로 SDK admission 확인
5. public Rust SDK Listener/Connector로 local 및 one-hop Pipe 확인
6. Gateway pod 하나 삭제
   -> 해당 Pipe 종료, SDK session 재연결, Listener current binding 재등록
7. RT shard pod 하나 삭제
   -> READY-empty, 해당 shard current snapshot 재등록, established Pipe 유지
8. PVC/emptyDir와 chart-managed Secret이 없음을 확인
```
...
않는다. `Recreate`는 정상 Deployment upgrade 순서이며 partition된 node의 process fencing을
증명하지 않는다.
