...
headless peer/RT Services + SDK ClusterIP Service
ShardDirectory ConfigMap
external credential Secret + external TLS Secret
```
...
- SDK port만 ClusterIP entry로 제공하고 peer/RT는 cluster 내부다.
- TLS Secret은 read-only file로 mount하고 production env에 insecure switch를 노출하지 않는다.
