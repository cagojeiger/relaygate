...
| `AC-HELM-08` | schema는 replica/shard 수와 shard 상한, port, image, log format, Secret reference의 잘못된 값을 거절한다. render는 chart-managed extraEnv 충돌과 생성된 pod FQDN의 DNS 길이 초과를 거절한다. |
