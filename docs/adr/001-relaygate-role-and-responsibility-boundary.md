...
- Controller는 영속 Raft에 현재 `GatewaySession`과 exact route로 구성된 제어 영역 경로 목록만 저장한다. 이 현재 상태만의 예외와 복구 경계는 [ADR 002](002-current-state-cluster-and-recovery.md)가 정의한다.
