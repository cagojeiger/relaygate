...
- SDK 세션, 로컬 Listener 바인딩, Pipe, 버퍼, payload는 Gateway 프로세스 메모리에만 존재한다.
- Controller는 영속 Raft에 현재 `GatewaySession`과 exact route로 구성된 제어 영역 경로 목록만 저장한다. 이 현재 상태만의 예외와 복구 경계는 [ADR 003](003-current-state-cluster-and-recovery.md)이 정의한다.
...
- RelayGate는 현재 도달 가능성과 연결 상태만 다루며 애플리케이션 결과나 payload 이력을 보존하지 않는다.
