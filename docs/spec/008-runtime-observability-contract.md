...
정상 close와 `PeerObservation`이 없는 operation에는 존재하지 않는 오류 field를 합성하지 않는다.
...
| `OBS-006` | SDK와 Gateway terminal failure event는 기존 결과에 있는 `error_code`와, 정의된 경우 `observation`을 바꾸지 않고 관찰해야 한다. 정상 close와 `PeerObservation`이 없는 operation에는 오류 field를 합성하지 않는다. |
