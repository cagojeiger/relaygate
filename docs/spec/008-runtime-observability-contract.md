...
terminal failure event는 기존 `error_code`와 `observation`을 함께 기록하고, 정상 close는 존재하지
않는 오류 field를 합성하지 않는다.
...
| `OBS-006` | SDK와 Gateway terminal failure event는 기존 `error_code`와 `observation`을 바꾸지 않고 관찰해야 한다. 정상 close는 오류 field를 합성하지 않는다. |
