...
| `OBS-019` | peer handshake 결과와 PeerTransport 종료는 bounded direction, outcome, code와 reason으로 집계해야 한다. 경쟁하는 종료 원인이 있어도 하나의 PeerTransport는 terminal counter와 event를 정확히 한 번만 만들고, actor가 강제 종료한 원인을 보존하며 idle retirement와 정상 shutdown을 active failure와 구분해야 한다. |
