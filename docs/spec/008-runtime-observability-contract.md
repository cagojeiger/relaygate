...
실행하고 SDK TLS를 유지한다. 내부 key 인증은 유지되지만 키와 payload의 전송 암호화는 없다.
...
| `SEC-004` | internal mTLS는 certificate, `relaygate/2` ALPN과 logical identity를 함께 검증한다. 명시적 plaintext는 logical handshake/key만 검증한다. |
