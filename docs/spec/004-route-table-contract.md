...
| `RT-009` | Resolve 응답은 exact Destination의 current live BindingSet으로 한정하며, 빈 결과는 `NOT_FOUND`다. |
...
SHA-256 digest의 첫 8바이트를 big-endian `u64`로 해석한 값을 shard 수로 나눈 나머지가 authority index입니다.
