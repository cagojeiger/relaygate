...
| `REG-005` | Gateway는 최초 및 recovery 등록 요청마다 `ClientId`와 `ClientKey`의 등록 권한을 확인해야 한다. 검증 실패 시 local binding과 RT mapping을 만들지 않는다. |
...
| `REG-011` | 한 session이 여러 shard에 걸치면 Gateway는 shard마다 독립된 active `LeaseId`, revision과 sync 상태를 관리해야 한다. shard의 공유 RT 연결 상실은 그 연결을 공유하는 해당 shard registration을 보수적으로 `UNSYNCED`로 만들 수 있지만 다른 shard registration이나 local binding을 변경하지 않는다. |
...
| `REG-014` | Gateway는 process가 session을 수락하기 전에 configured `ClientId`마다 정확히 하나의 `ClientKey`를 startup configuration에서 로드하고 process 수명 동안 바꾸지 않아야 한다. runtime key 발급, hot reload, 동시 rotation과 active binding revocation은 이 계약에 없다. key 변경은 새 Gateway process configuration으로만 적용한다. |
| `REG-015` | RT shard 연결 상실은 그 연결을 공유하는 active key를 각각 `UNSYNCED`로 만들 수 있고, stale lease 또는 개별 registration 실패는 해당 key를 `UNSYNCED`로 만든다. 어느 경우에도 `ACTIVE` local binding과 ListenerSession을 제거해서는 안 된다. lease-bound `Update` 또는 `KeepAlive`의 `FAILED_PRECONDITION`은 원인을 wire에서 구분할 수 없으므로 의심되는 `LeaseId`를 버리고 idempotent `Register`를 한 번 수행해 RT의 current active lease를 확인할 수 있다. `Register`가 성공하면 반환된 lease와 revision으로 current snapshot을 이어서 publish한다. 그 다음 lease-bound operation도 `FAILED_PRECONDITION`이면 치유 불가능한 전제조건 실패로 보고 terminal로 끝내며 두 번째 `Register` 판별을 시작하지 않는다. `Register` 자체의 `FAILED_PRECONDITION`도 terminal이다. 성공한 `Update` 또는 `KeepAlive`만 이 판별 한도를 초기화한다. transient failure만 같은 process generation으로 자동 재시도할 수 있다. |
...
| `REG-019` | Gateway는 모든 RT registration operation에 자신의 `ShardDirectoryGeneration`을 보내야 한다. 최초 `Register`의 generation mismatch는 즉시 terminal이다. lease-bound operation에서 처음 받은 `FAILED_PRECONDITION`에는 `REG-015`의 idempotent `Register` 판별을 정확히 한 번 허용한다. 그 `Register`가 거절되거나 성공 뒤 다음 lease-bound operation도 거절되면 key를 `UNSYNCED`로 유지한 채 같은 process configuration에서 더 재시도하지 않는다. local binding은 제거하지 않는다. |
...
| ShardDirectory generation mismatch | binding `ACTIVE`, key `UNSYNCED`, 최초 `Register` 거절 또는 lease-bound 실패 뒤 `Register` 판별 1회로 terminal, process restart 필요 | operation 거절, state 불변 |
