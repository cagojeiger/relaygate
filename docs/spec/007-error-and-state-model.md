...
| `STATE-DRAIN-001` | Gateway drain | `RUNNING` | normal shutdown 시작 | `DRAINING` | SDK listener를 닫고 current Listener publication을 RT에서 철회하며 신규 session, `REGISTER`, SDK `OPEN`과 peer `OPEN`을 `UNAVAILABLE`로 거절. Gateway-local binding과 기존 pending attempt·Pipe는 계속 유지 |
| `STATE-DRAIN-002` | Gateway drain | `DRAINING` | pending offer, remote open attempt와 live Pipe가 모두 0 | `STOPPING` | SDK session과 distributed runtime 종료 |
| `STATE-DRAIN-003` | Gateway drain | `DRAINING` | configured drain timeout | `STOPPING` | 남은 session·attempt·Pipe를 transport-loss cleanup으로 강제 종료. replay·migration·resume 없음 |
...
| RT restart | RT lease와 mapping 소실, Gateway의 `RouteDependencyHealth`는 `DEGRADED`로 수렴 | `ProcessLiveness`, `SdkAdmissionReadiness`, Gateway local binding, established Pipe | Gateway와 shard별 jitter가 적용된 bounded backoff로 재연결한다. READY-empty 수렴 중 `NOT_FOUND`, 새 lease 등록과 current snapshot update 뒤 `READY` 복구 |
...
| Gateway normal shutdown | 신규 admission과 새 `REGISTER`·SDK/peer `OPEN` 중단, current Listener publication 철회, 기존 pending attempt와 Pipe를 configured deadline까지 drain | 다른 Gateway process, Gateway-local binding과 기존 Pipe | active work가 0이면 즉시 종료하고 deadline이면 남은 session·Pipe를 강제 종료. 철회 실패 시 RT lease expiry가 stale mapping을 제거한다. SDK 재연결은 jitter된 bounded backoff를 사용하며 기존 Pipe는 이동하지 않음 |
...
36. Gateway normal shutdown은 신규 session, `REGISTER`, SDK `OPEN`과 peer `OPEN`을 중단한 뒤 이미 시작된 pending attempt와 live Pipe만 configured deadline까지 유지한다. active work가 0이면 즉시, deadline이면 강제로 enclosing session과 distributed runtime을 종료한다. 이 drain은 Pipe replay, migration 또는 resume를 허용하지 않는다.
