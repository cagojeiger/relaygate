...
| `PEER-010` | peer OPEN은 exact RouteAddress, Binding과 origin open identity를 current state와 대조한다. |
...
RouteAddress와 open identity로 축소되며 Owner Gateway는 current Binding을 다시 대조합니다.
