...
- 반복되는 session failure와 `REGISTER` response timeout은 bounded exponential reconnect backoff를 따라야 한다. 각 SDK runtime은 매 단계 delay를 해당 backoff의 `2/3..1` 범위에서 jitter하여 동시 재연결을 분산한다. TCP session을 만들었다는 이유만으로 failure streak을 즉시 초기화하지 않고, current desired registration의 terminal 성공을 관찰한 뒤 정상 수렴으로 취급한다.
...
- **`SDK-030`**: managed reconnect는 SDK runtime별 entropy를 사용해 현재 exponential backoff 단계의 `2/3..1` 범위에서 매 시도 delay를 jitter해야 한다. delay는 configured maximum backoff를 넘지 않아야 하며 기존 Connector 안정 구간과 Listener recovery registration 성공의 reset 조건을 바꾸지 않아야 한다.
