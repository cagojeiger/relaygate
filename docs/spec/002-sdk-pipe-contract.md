...
Listener SDK runtime
  ...
  └── ClientId -> non-CLOSED Listener handle 0..1
```

Listener 등록 API와 lifecycle은 SPEC 003이 정의한다. 이 문서는 등록된 Listener의 연결 수신 계약만 소유한다.

Listener의 최초 등록 성공은 `ClientKey` 검증과 Gateway-local binding 설치가 끝나 `ACTIVE`가 된 뒤 반환한다. RT publication은 별도의 `UNSYNCED/SYNCED` 상태이며, 등록 성공은 remote Gateway에서 이미 발견 가능하다는 보장이 아니다. shard-local projection의 일시적 상실은 이미 성공한 Listener handle을 새로 만들거나 local admission을 중단시키지 않는다.
...
한 Listener SDK runtime에서 같은 `ClientId`의 non-`CLOSED` Listener handle을 두 개 만들 수 없다. 생성은 runtime의 `ClientId` index에서 atomic하게 직렬화한다. 이미 handle이 있으면 두 번째 생성은 `ALREADY_EXISTS`로 끝나며 새 registration, binding, incoming queue를 만들지 않는다. 기존 handle이 `CLOSED`가 된 뒤에는 같은 `ClientId`로 새 handle을 만들 수 있고, 새 session registration은 재사용하지 않은 새 `BindingId`를 사용한다.
...
- queue 또는 write capacity를 확보하지 못한 작업은 자신의 deadline 안에서 대기하거나 partial result 또는 terminal failure를 반환해야 하며 bytes를 조용히 버려서는 안 된다.
...
- `Listener` handle들은 일시적인 Gateway 단절 동안 각자의 desired `ClientId`를 유지한다. SDK runtime은 current desired set을 source of truth로 삼고 shared session을 다시 만든 뒤 살아 있는 handle의 registration을 전부 다시 등록한다. 일시적인 내부 queue 포화나 과거 등록·해제 명령 유실 때문에 current desired handle을 누락해서는 안 된다.
- credential 거절이나 등록 권한 폐기는 transport 단절로 취급하지 않는다. 영향받은 Listener handle은 관련 credential 또는 configuration이 바뀔 때까지 자동 재등록하지 않는다.
...
- **`SDK-016`**: Listener의 최초 등록 성공은 `ClientKey` 검증과 Gateway-local binding 설치가 끝난 뒤 반환해야 한다. RT publication 성공 여부는 별도의 상태이며, 등록 성공을 remote discovery 완료로 해석해서는 안 된다.
- **`SDK-017`**: credential 거절 또는 등록 권한 폐기로 `BLOCKED`가 된 Listener handle은 관련 credential이나 configuration 변경 전까지 자동 재등록해서는 안 되며 sibling handle, shared session과 `BLOCKED` 전 admission을 마친 queued·accepted Pipe를 유지해야 한다.
...
- **`SDK-019`**: 같은 Listener SDK runtime에서 동일 `ClientId`의 non-`CLOSED` Listener handle을 동시에 생성하려는 호출은 하나만 성공해야 한다. 나머지는 `ALREADY_EXISTS`이고 새 registration, binding 또는 incoming queue를 만들지 않아야 하며, 기존 handle이 `CLOSED`가 된 뒤에는 새 handle 생성이 가능해야 한다.
