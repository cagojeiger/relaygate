...
fn equal_destination_names_in_different_namespaces_are_isolated() -> Result<(), RouteTableError> {
    ...
    let lease_id = shard
        ...
        .lease_id();
    let red = "red/shared".parse::<Destination>()?;
    let blue = "blue/shared".parse::<Destination>()?;
    let red_binding = BindingProjection::new(
        red.clone(),
        gateway_id,
        relay_session_id,
    ...
    let blue_binding = BindingProjection::new(
        blue.clone(),
        gateway_id,
        relay_session_id,
    ...
    shard.update(
        ...
        snapshot([red_binding.clone(), blue_binding.clone()])?,
        now,
    )?;
...
-> Result<(), RouteTableError> {
    ...
    let alpha = projection("alpha", gateway_id, session_id, binding(100))?;
    let beta = projection("beta", gateway_id, session_id, binding(101))?;
...
fn revision_rules_are_monotonic_atomic_and_idempotent() -> Result<(), RouteTableError> {
    ...
    let lease = shard
        ...
        .lease_id();
    let alpha = projection("alpha", gateway_id, session_id, binding(200))?;
    let beta = projection("beta", gateway_id, session_id, binding(201))?;
