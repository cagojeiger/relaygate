...
impl GatewayPeerConfig {
    pub fn new(local_gateway_name: impl Into<String>) -> Result<Self, PeerConfigError> {
        let config = Self {
    ...
    #[cfg(test)]
    pub fn with_connect_gate(mut self, gate: ConnectGate) -> Self {
        self.connect_gate = Some(gate);
        self
    }

    #[cfg(test)]
    pub fn with_open_commit_gate(mut self, gate: OpenCommitGate) -> Self {
...
fn config_error(error: PeerError) -> PeerConfigError {
    PeerConfigError(error.to_string())
}
