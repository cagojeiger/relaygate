...
func (s *Service) Forward(stream grpc.BidiStreamingServer[gatewayv1.ForwardRequest, gatewayv1.ForwardResponse]) error {
	...
	if err := s.sendResponse(stream.Context(), outbound, &gatewayv1.ForwardResponse{Message: &gatewayv1.ForwardResponse_Accepted{
		Accepted: &gatewayv1.ForwardAccepted{
			...
			Binding:   bindingSlotToProto(result.Binding),
		},
	}}); err != nil {
		return err
	}
...
func validOwnerResult(open authority.OpenContext, result opening.Result) bool {
	return result.AttemptID == open.AttemptID && result.PipeID != "" && len(result.PipeID) <= controlstate.MaxIdentityBytes && sameBinding(result.Binding, open.Binding)
}
...
func (e *serverCallerEndpoint) TerminatePipe(parent context.Context, pipeID string) error {
	e.pipeMu.Lock()
	expected := e.pipeID
	if expected == "" && pipeID != "" && len(pipeID) <= controlstate.MaxIdentityBytes {
		e.pipeID = pipeID
		expected = pipeID
	}
	e.pipeMu.Unlock()
