...
func (n *Node) EnsureEpoch(ctx context.Context, clusterEpoch string, maxDistinctKeys, maxDistinctGatewayIDs uint64) (controlstate.ApplyResult, error) {
	command, err := controlstate.EncodeInitializeEpoch(controlstate.InitializeEpoch{
		ClusterEpoch:                   clusterEpoch,
		MaxDistinctBindingKeysPerEpoch: maxDistinctKeys,
		MaxDistinctGatewayIDsPerEpoch:  maxDistinctGatewayIDs,
	})
	if err != nil {
	...
	for {
		...
		if state.ClusterEpoch != "" {
			if state.ClusterEpoch != clusterEpoch ||
				state.MaxDistinctBindingKeysPerEpoch != maxDistinctKeys ||
				state.MaxDistinctGatewayIDsPerEpoch != maxDistinctGatewayIDs {
...
func (n *Node) Apply(ctx context.Context, command []byte) (controlstate.ApplyResult, error) {
	...
	if len(command) == 0 {
		return controlstate.ApplyResult{}, fmt.Errorf("control command is empty")
	}
	if len(command) > n.config.MaxCommandBytes {
		return controlstate.ApplyResult{}, fmt.Errorf("control command is %d bytes; limit is %d", len(command), n.config.MaxCommandBytes)
	}
	...
	case result := <-completed:
		if result.err != nil {
			...
			return controlstate.ApplyResult{}, fmt.Errorf("apply control command: %w", result.err)
		}
...
func (n *Node) LookupGateway(gatewayID string) controlstate.GatewaySlot {
