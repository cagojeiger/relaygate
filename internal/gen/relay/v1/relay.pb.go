// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.12
// 	protoc        v6.33.4
// source: proto/relaygate/relay/v1/relay.proto

package relayv1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ListenerBindingFailure int32

const (
	ListenerBindingFailure_LISTENER_BINDING_FAILURE_UNSPECIFIED      ListenerBindingFailure = 0
	ListenerBindingFailure_LISTENER_BINDING_FAILURE_INVALID_REQUEST  ListenerBindingFailure = 1
	ListenerBindingFailure_LISTENER_BINDING_FAILURE_CAPACITY_REACHED ListenerBindingFailure = 2
	ListenerBindingFailure_LISTENER_BINDING_FAILURE_CONFLICT         ListenerBindingFailure = 3
	ListenerBindingFailure_LISTENER_BINDING_FAILURE_UNAVAILABLE      ListenerBindingFailure = 4
)

// Enum value maps for ListenerBindingFailure.
var (
	ListenerBindingFailure_name = map[int32]string{
		0: "LISTENER_BINDING_FAILURE_UNSPECIFIED",
		1: "LISTENER_BINDING_FAILURE_INVALID_REQUEST",
		2: "LISTENER_BINDING_FAILURE_CAPACITY_REACHED",
		3: "LISTENER_BINDING_FAILURE_CONFLICT",
		4: "LISTENER_BINDING_FAILURE_UNAVAILABLE",
	}
	ListenerBindingFailure_value = map[string]int32{
		"LISTENER_BINDING_FAILURE_UNSPECIFIED":      0,
		"LISTENER_BINDING_FAILURE_INVALID_REQUEST":  1,
		"LISTENER_BINDING_FAILURE_CAPACITY_REACHED": 2,
		"LISTENER_BINDING_FAILURE_CONFLICT":         3,
		"LISTENER_BINDING_FAILURE_UNAVAILABLE":      4,
	}
)

func (x ListenerBindingFailure) Enum() *ListenerBindingFailure {
	p := new(ListenerBindingFailure)
	*p = x
	return p
}

func (x ListenerBindingFailure) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ListenerBindingFailure) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_relaygate_relay_v1_relay_proto_enumTypes[0].Descriptor()
}

func (ListenerBindingFailure) Type() protoreflect.EnumType {
	return &file_proto_relaygate_relay_v1_relay_proto_enumTypes[0]
}

func (x ListenerBindingFailure) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ListenerBindingFailure.Descriptor instead.
func (ListenerBindingFailure) EnumDescriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{0}
}

type OpenFailure int32

const (
	OpenFailure_OPEN_FAILURE_UNSPECIFIED       OpenFailure = 0
	OpenFailure_OPEN_FAILURE_INVALID_REQUEST   OpenFailure = 1
	OpenFailure_OPEN_FAILURE_ROUTE_NOT_FOUND   OpenFailure = 2
	OpenFailure_OPEN_FAILURE_UNAVAILABLE       OpenFailure = 3
	OpenFailure_OPEN_FAILURE_CAPACITY_REACHED  OpenFailure = 4
	OpenFailure_OPEN_FAILURE_LISTENER_REJECTED OpenFailure = 5
	OpenFailure_OPEN_FAILURE_DEADLINE_EXCEEDED OpenFailure = 6
	OpenFailure_OPEN_FAILURE_CANCELLED         OpenFailure = 7
)

// Enum value maps for OpenFailure.
var (
	OpenFailure_name = map[int32]string{
		0: "OPEN_FAILURE_UNSPECIFIED",
		1: "OPEN_FAILURE_INVALID_REQUEST",
		2: "OPEN_FAILURE_ROUTE_NOT_FOUND",
		3: "OPEN_FAILURE_UNAVAILABLE",
		4: "OPEN_FAILURE_CAPACITY_REACHED",
		5: "OPEN_FAILURE_LISTENER_REJECTED",
		6: "OPEN_FAILURE_DEADLINE_EXCEEDED",
		7: "OPEN_FAILURE_CANCELLED",
	}
	OpenFailure_value = map[string]int32{
		"OPEN_FAILURE_UNSPECIFIED":       0,
		"OPEN_FAILURE_INVALID_REQUEST":   1,
		"OPEN_FAILURE_ROUTE_NOT_FOUND":   2,
		"OPEN_FAILURE_UNAVAILABLE":       3,
		"OPEN_FAILURE_CAPACITY_REACHED":  4,
		"OPEN_FAILURE_LISTENER_REJECTED": 5,
		"OPEN_FAILURE_DEADLINE_EXCEEDED": 6,
		"OPEN_FAILURE_CANCELLED":         7,
	}
)

func (x OpenFailure) Enum() *OpenFailure {
	p := new(OpenFailure)
	*p = x
	return p
}

func (x OpenFailure) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (OpenFailure) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_relaygate_relay_v1_relay_proto_enumTypes[1].Descriptor()
}

func (OpenFailure) Type() protoreflect.EnumType {
	return &file_proto_relaygate_relay_v1_relay_proto_enumTypes[1]
}

func (x OpenFailure) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use OpenFailure.Descriptor instead.
func (OpenFailure) EnumDescriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{1}
}

type OpenRequestFailure int32

const (
	OpenRequestFailure_OPEN_REQUEST_FAILURE_UNSPECIFIED         OpenRequestFailure = 0
	OpenRequestFailure_OPEN_REQUEST_FAILURE_DUPLICATE_IN_FLIGHT OpenRequestFailure = 1
)

// Enum value maps for OpenRequestFailure.
var (
	OpenRequestFailure_name = map[int32]string{
		0: "OPEN_REQUEST_FAILURE_UNSPECIFIED",
		1: "OPEN_REQUEST_FAILURE_DUPLICATE_IN_FLIGHT",
	}
	OpenRequestFailure_value = map[string]int32{
		"OPEN_REQUEST_FAILURE_UNSPECIFIED":         0,
		"OPEN_REQUEST_FAILURE_DUPLICATE_IN_FLIGHT": 1,
	}
)

func (x OpenRequestFailure) Enum() *OpenRequestFailure {
	p := new(OpenRequestFailure)
	*p = x
	return p
}

func (x OpenRequestFailure) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (OpenRequestFailure) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_relaygate_relay_v1_relay_proto_enumTypes[2].Descriptor()
}

func (OpenRequestFailure) Type() protoreflect.EnumType {
	return &file_proto_relaygate_relay_v1_relay_proto_enumTypes[2]
}

func (x OpenRequestFailure) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use OpenRequestFailure.Descriptor instead.
func (OpenRequestFailure) EnumDescriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{2}
}

type ListenerDecisionFailure int32

const (
	ListenerDecisionFailure_LISTENER_DECISION_FAILURE_UNSPECIFIED         ListenerDecisionFailure = 0
	ListenerDecisionFailure_LISTENER_DECISION_FAILURE_INVALID_REQUEST     ListenerDecisionFailure = 1
	ListenerDecisionFailure_LISTENER_DECISION_FAILURE_ATTEMPT_NOT_PENDING ListenerDecisionFailure = 2
	ListenerDecisionFailure_LISTENER_DECISION_FAILURE_WRONG_PHASE         ListenerDecisionFailure = 3
)

// Enum value maps for ListenerDecisionFailure.
var (
	ListenerDecisionFailure_name = map[int32]string{
		0: "LISTENER_DECISION_FAILURE_UNSPECIFIED",
		1: "LISTENER_DECISION_FAILURE_INVALID_REQUEST",
		2: "LISTENER_DECISION_FAILURE_ATTEMPT_NOT_PENDING",
		3: "LISTENER_DECISION_FAILURE_WRONG_PHASE",
	}
	ListenerDecisionFailure_value = map[string]int32{
		"LISTENER_DECISION_FAILURE_UNSPECIFIED":         0,
		"LISTENER_DECISION_FAILURE_INVALID_REQUEST":     1,
		"LISTENER_DECISION_FAILURE_ATTEMPT_NOT_PENDING": 2,
		"LISTENER_DECISION_FAILURE_WRONG_PHASE":         3,
	}
)

func (x ListenerDecisionFailure) Enum() *ListenerDecisionFailure {
	p := new(ListenerDecisionFailure)
	*p = x
	return p
}

func (x ListenerDecisionFailure) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ListenerDecisionFailure) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_relaygate_relay_v1_relay_proto_enumTypes[3].Descriptor()
}

func (ListenerDecisionFailure) Type() protoreflect.EnumType {
	return &file_proto_relaygate_relay_v1_relay_proto_enumTypes[3]
}

func (x ListenerDecisionFailure) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ListenerDecisionFailure.Descriptor instead.
func (ListenerDecisionFailure) EnumDescriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{3}
}

type PipePayloadFailure int32

const (
	PipePayloadFailure_PIPE_PAYLOAD_FAILURE_UNSPECIFIED     PipePayloadFailure = 0
	PipePayloadFailure_PIPE_PAYLOAD_FAILURE_INVALID_REQUEST PipePayloadFailure = 1
	PipePayloadFailure_PIPE_PAYLOAD_FAILURE_NOT_OWNED       PipePayloadFailure = 2
	PipePayloadFailure_PIPE_PAYLOAD_FAILURE_BACKPRESSURE    PipePayloadFailure = 3
	PipePayloadFailure_PIPE_PAYLOAD_FAILURE_UNAVAILABLE     PipePayloadFailure = 4
)

// Enum value maps for PipePayloadFailure.
var (
	PipePayloadFailure_name = map[int32]string{
		0: "PIPE_PAYLOAD_FAILURE_UNSPECIFIED",
		1: "PIPE_PAYLOAD_FAILURE_INVALID_REQUEST",
		2: "PIPE_PAYLOAD_FAILURE_NOT_OWNED",
		3: "PIPE_PAYLOAD_FAILURE_BACKPRESSURE",
		4: "PIPE_PAYLOAD_FAILURE_UNAVAILABLE",
	}
	PipePayloadFailure_value = map[string]int32{
		"PIPE_PAYLOAD_FAILURE_UNSPECIFIED":     0,
		"PIPE_PAYLOAD_FAILURE_INVALID_REQUEST": 1,
		"PIPE_PAYLOAD_FAILURE_NOT_OWNED":       2,
		"PIPE_PAYLOAD_FAILURE_BACKPRESSURE":    3,
		"PIPE_PAYLOAD_FAILURE_UNAVAILABLE":     4,
	}
)

func (x PipePayloadFailure) Enum() *PipePayloadFailure {
	p := new(PipePayloadFailure)
	*p = x
	return p
}

func (x PipePayloadFailure) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (PipePayloadFailure) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_relaygate_relay_v1_relay_proto_enumTypes[4].Descriptor()
}

func (PipePayloadFailure) Type() protoreflect.EnumType {
	return &file_proto_relaygate_relay_v1_relay_proto_enumTypes[4]
}

func (x PipePayloadFailure) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use PipePayloadFailure.Descriptor instead.
func (PipePayloadFailure) EnumDescriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{4}
}

type ConnectRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Message:
	//
	//	*ConnectRequest_Authenticate
	//	*ConnectRequest_BindListener
	//	*ConnectRequest_UnbindListener
	//	*ConnectRequest_Open
	//	*ConnectRequest_ListenerAccept
	//	*ConnectRequest_ListenerReject
	//	*ConnectRequest_ListenerConfirmed
	//	*ConnectRequest_CancelOpen
	//	*ConnectRequest_ClosePipe
	//	*ConnectRequest_PipePayload
	//	*ConnectRequest_PipePayloadReceived
	//	*ConnectRequest_PipePayloadRejected
	Message       isConnectRequest_Message `protobuf_oneof:"message"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConnectRequest) Reset() {
	*x = ConnectRequest{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConnectRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConnectRequest) ProtoMessage() {}

func (x *ConnectRequest) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConnectRequest.ProtoReflect.Descriptor instead.
func (*ConnectRequest) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{0}
}

func (x *ConnectRequest) GetMessage() isConnectRequest_Message {
	if x != nil {
		return x.Message
	}
	return nil
}

func (x *ConnectRequest) GetAuthenticate() *Authenticate {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_Authenticate); ok {
			return x.Authenticate
		}
	}
	return nil
}

func (x *ConnectRequest) GetBindListener() *BindListener {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_BindListener); ok {
			return x.BindListener
		}
	}
	return nil
}

func (x *ConnectRequest) GetUnbindListener() *UnbindListener {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_UnbindListener); ok {
			return x.UnbindListener
		}
	}
	return nil
}

func (x *ConnectRequest) GetOpen() *Open {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_Open); ok {
			return x.Open
		}
	}
	return nil
}

func (x *ConnectRequest) GetListenerAccept() *ListenerAccept {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_ListenerAccept); ok {
			return x.ListenerAccept
		}
	}
	return nil
}

func (x *ConnectRequest) GetListenerReject() *ListenerReject {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_ListenerReject); ok {
			return x.ListenerReject
		}
	}
	return nil
}

func (x *ConnectRequest) GetListenerConfirmed() *ListenerConfirmed {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_ListenerConfirmed); ok {
			return x.ListenerConfirmed
		}
	}
	return nil
}

func (x *ConnectRequest) GetCancelOpen() *CancelOpen {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_CancelOpen); ok {
			return x.CancelOpen
		}
	}
	return nil
}

func (x *ConnectRequest) GetClosePipe() *ClosePipe {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_ClosePipe); ok {
			return x.ClosePipe
		}
	}
	return nil
}

func (x *ConnectRequest) GetPipePayload() *PipePayload {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_PipePayload); ok {
			return x.PipePayload
		}
	}
	return nil
}

func (x *ConnectRequest) GetPipePayloadReceived() *PipePayloadReceived {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_PipePayloadReceived); ok {
			return x.PipePayloadReceived
		}
	}
	return nil
}

func (x *ConnectRequest) GetPipePayloadRejected() *PipePayloadRejected {
	if x != nil {
		if x, ok := x.Message.(*ConnectRequest_PipePayloadRejected); ok {
			return x.PipePayloadRejected
		}
	}
	return nil
}

type isConnectRequest_Message interface {
	isConnectRequest_Message()
}

type ConnectRequest_Authenticate struct {
	Authenticate *Authenticate `protobuf:"bytes,1,opt,name=authenticate,proto3,oneof"`
}

type ConnectRequest_BindListener struct {
	BindListener *BindListener `protobuf:"bytes,2,opt,name=bind_listener,json=bindListener,proto3,oneof"`
}

type ConnectRequest_UnbindListener struct {
	UnbindListener *UnbindListener `protobuf:"bytes,3,opt,name=unbind_listener,json=unbindListener,proto3,oneof"`
}

type ConnectRequest_Open struct {
	Open *Open `protobuf:"bytes,4,opt,name=open,proto3,oneof"`
}

type ConnectRequest_ListenerAccept struct {
	ListenerAccept *ListenerAccept `protobuf:"bytes,5,opt,name=listener_accept,json=listenerAccept,proto3,oneof"`
}

type ConnectRequest_ListenerReject struct {
	ListenerReject *ListenerReject `protobuf:"bytes,6,opt,name=listener_reject,json=listenerReject,proto3,oneof"`
}

type ConnectRequest_ListenerConfirmed struct {
	ListenerConfirmed *ListenerConfirmed `protobuf:"bytes,7,opt,name=listener_confirmed,json=listenerConfirmed,proto3,oneof"`
}

type ConnectRequest_CancelOpen struct {
	CancelOpen *CancelOpen `protobuf:"bytes,8,opt,name=cancel_open,json=cancelOpen,proto3,oneof"`
}

type ConnectRequest_ClosePipe struct {
	ClosePipe *ClosePipe `protobuf:"bytes,9,opt,name=close_pipe,json=closePipe,proto3,oneof"`
}

type ConnectRequest_PipePayload struct {
	PipePayload *PipePayload `protobuf:"bytes,10,opt,name=pipe_payload,json=pipePayload,proto3,oneof"`
}

type ConnectRequest_PipePayloadReceived struct {
	PipePayloadReceived *PipePayloadReceived `protobuf:"bytes,11,opt,name=pipe_payload_received,json=pipePayloadReceived,proto3,oneof"`
}

type ConnectRequest_PipePayloadRejected struct {
	PipePayloadRejected *PipePayloadRejected `protobuf:"bytes,12,opt,name=pipe_payload_rejected,json=pipePayloadRejected,proto3,oneof"`
}

func (*ConnectRequest_Authenticate) isConnectRequest_Message() {}

func (*ConnectRequest_BindListener) isConnectRequest_Message() {}

func (*ConnectRequest_UnbindListener) isConnectRequest_Message() {}

func (*ConnectRequest_Open) isConnectRequest_Message() {}

func (*ConnectRequest_ListenerAccept) isConnectRequest_Message() {}

func (*ConnectRequest_ListenerReject) isConnectRequest_Message() {}

func (*ConnectRequest_ListenerConfirmed) isConnectRequest_Message() {}

func (*ConnectRequest_CancelOpen) isConnectRequest_Message() {}

func (*ConnectRequest_ClosePipe) isConnectRequest_Message() {}

func (*ConnectRequest_PipePayload) isConnectRequest_Message() {}

func (*ConnectRequest_PipePayloadReceived) isConnectRequest_Message() {}

func (*ConnectRequest_PipePayloadRejected) isConnectRequest_Message() {}

type ConnectResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Message:
	//
	//	*ConnectResponse_ClientSessionOpened
	//	*ConnectResponse_ListenerBound
	//	*ConnectResponse_ListenerUnbound
	//	*ConnectResponse_ListenerOffer
	//	*ConnectResponse_ListenerEstablished
	//	*ConnectResponse_ListenerTerminated
	//	*ConnectResponse_PipeOpened
	//	*ConnectResponse_PipeOpenFailed
	//	*ConnectResponse_PipeOpenUnknown
	//	*ConnectResponse_ListenerDecisionRejected
	//	*ConnectResponse_OpenCancelAcknowledged
	//	*ConnectResponse_PipeCloseAcknowledged
	//	*ConnectResponse_OpenRequestRejected
	//	*ConnectResponse_PipePayload
	//	*ConnectResponse_PipeTerminated
	//	*ConnectResponse_PipePayloadRejected
	//	*ConnectResponse_ListenerConfirmationAcknowledged
	//	*ConnectResponse_ListenerBindFailed
	//	*ConnectResponse_ListenerUnbindFailed
	//	*ConnectResponse_PipePayloadReceived
	Message       isConnectResponse_Message `protobuf_oneof:"message"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ConnectResponse) Reset() {
	*x = ConnectResponse{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ConnectResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ConnectResponse) ProtoMessage() {}

func (x *ConnectResponse) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ConnectResponse.ProtoReflect.Descriptor instead.
func (*ConnectResponse) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{1}
}

func (x *ConnectResponse) GetMessage() isConnectResponse_Message {
	if x != nil {
		return x.Message
	}
	return nil
}

func (x *ConnectResponse) GetClientSessionOpened() *ClientSessionOpened {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ClientSessionOpened); ok {
			return x.ClientSessionOpened
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerBound() *ListenerBound {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerBound); ok {
			return x.ListenerBound
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerUnbound() *ListenerUnbound {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerUnbound); ok {
			return x.ListenerUnbound
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerOffer() *ListenerOffer {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerOffer); ok {
			return x.ListenerOffer
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerEstablished() *ListenerEstablished {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerEstablished); ok {
			return x.ListenerEstablished
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerTerminated() *ListenerTerminated {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerTerminated); ok {
			return x.ListenerTerminated
		}
	}
	return nil
}

func (x *ConnectResponse) GetPipeOpened() *PipeOpened {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_PipeOpened); ok {
			return x.PipeOpened
		}
	}
	return nil
}

func (x *ConnectResponse) GetPipeOpenFailed() *PipeOpenFailed {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_PipeOpenFailed); ok {
			return x.PipeOpenFailed
		}
	}
	return nil
}

func (x *ConnectResponse) GetPipeOpenUnknown() *PipeOpenUnknown {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_PipeOpenUnknown); ok {
			return x.PipeOpenUnknown
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerDecisionRejected() *ListenerDecisionRejected {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerDecisionRejected); ok {
			return x.ListenerDecisionRejected
		}
	}
	return nil
}

func (x *ConnectResponse) GetOpenCancelAcknowledged() *OpenCancelAcknowledged {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_OpenCancelAcknowledged); ok {
			return x.OpenCancelAcknowledged
		}
	}
	return nil
}

func (x *ConnectResponse) GetPipeCloseAcknowledged() *PipeCloseAcknowledged {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_PipeCloseAcknowledged); ok {
			return x.PipeCloseAcknowledged
		}
	}
	return nil
}

func (x *ConnectResponse) GetOpenRequestRejected() *OpenRequestRejected {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_OpenRequestRejected); ok {
			return x.OpenRequestRejected
		}
	}
	return nil
}

func (x *ConnectResponse) GetPipePayload() *PipePayload {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_PipePayload); ok {
			return x.PipePayload
		}
	}
	return nil
}

func (x *ConnectResponse) GetPipeTerminated() *PipeTerminated {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_PipeTerminated); ok {
			return x.PipeTerminated
		}
	}
	return nil
}

func (x *ConnectResponse) GetPipePayloadRejected() *PipePayloadRejected {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_PipePayloadRejected); ok {
			return x.PipePayloadRejected
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerConfirmationAcknowledged() *ListenerConfirmationAcknowledged {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerConfirmationAcknowledged); ok {
			return x.ListenerConfirmationAcknowledged
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerBindFailed() *ListenerBindFailed {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerBindFailed); ok {
			return x.ListenerBindFailed
		}
	}
	return nil
}

func (x *ConnectResponse) GetListenerUnbindFailed() *ListenerUnbindFailed {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_ListenerUnbindFailed); ok {
			return x.ListenerUnbindFailed
		}
	}
	return nil
}

func (x *ConnectResponse) GetPipePayloadReceived() *PipePayloadReceived {
	if x != nil {
		if x, ok := x.Message.(*ConnectResponse_PipePayloadReceived); ok {
			return x.PipePayloadReceived
		}
	}
	return nil
}

type isConnectResponse_Message interface {
	isConnectResponse_Message()
}

type ConnectResponse_ClientSessionOpened struct {
	ClientSessionOpened *ClientSessionOpened `protobuf:"bytes,1,opt,name=client_session_opened,json=clientSessionOpened,proto3,oneof"`
}

type ConnectResponse_ListenerBound struct {
	ListenerBound *ListenerBound `protobuf:"bytes,2,opt,name=listener_bound,json=listenerBound,proto3,oneof"`
}

type ConnectResponse_ListenerUnbound struct {
	ListenerUnbound *ListenerUnbound `protobuf:"bytes,3,opt,name=listener_unbound,json=listenerUnbound,proto3,oneof"`
}

type ConnectResponse_ListenerOffer struct {
	ListenerOffer *ListenerOffer `protobuf:"bytes,4,opt,name=listener_offer,json=listenerOffer,proto3,oneof"`
}

type ConnectResponse_ListenerEstablished struct {
	ListenerEstablished *ListenerEstablished `protobuf:"bytes,5,opt,name=listener_established,json=listenerEstablished,proto3,oneof"`
}

type ConnectResponse_ListenerTerminated struct {
	ListenerTerminated *ListenerTerminated `protobuf:"bytes,6,opt,name=listener_terminated,json=listenerTerminated,proto3,oneof"`
}

type ConnectResponse_PipeOpened struct {
	PipeOpened *PipeOpened `protobuf:"bytes,7,opt,name=pipe_opened,json=pipeOpened,proto3,oneof"`
}

type ConnectResponse_PipeOpenFailed struct {
	PipeOpenFailed *PipeOpenFailed `protobuf:"bytes,8,opt,name=pipe_open_failed,json=pipeOpenFailed,proto3,oneof"`
}

type ConnectResponse_PipeOpenUnknown struct {
	PipeOpenUnknown *PipeOpenUnknown `protobuf:"bytes,9,opt,name=pipe_open_unknown,json=pipeOpenUnknown,proto3,oneof"`
}

type ConnectResponse_ListenerDecisionRejected struct {
	ListenerDecisionRejected *ListenerDecisionRejected `protobuf:"bytes,10,opt,name=listener_decision_rejected,json=listenerDecisionRejected,proto3,oneof"`
}

type ConnectResponse_OpenCancelAcknowledged struct {
	OpenCancelAcknowledged *OpenCancelAcknowledged `protobuf:"bytes,11,opt,name=open_cancel_acknowledged,json=openCancelAcknowledged,proto3,oneof"`
}

type ConnectResponse_PipeCloseAcknowledged struct {
	PipeCloseAcknowledged *PipeCloseAcknowledged `protobuf:"bytes,12,opt,name=pipe_close_acknowledged,json=pipeCloseAcknowledged,proto3,oneof"`
}

type ConnectResponse_OpenRequestRejected struct {
	OpenRequestRejected *OpenRequestRejected `protobuf:"bytes,13,opt,name=open_request_rejected,json=openRequestRejected,proto3,oneof"`
}

type ConnectResponse_PipePayload struct {
	PipePayload *PipePayload `protobuf:"bytes,14,opt,name=pipe_payload,json=pipePayload,proto3,oneof"`
}

type ConnectResponse_PipeTerminated struct {
	PipeTerminated *PipeTerminated `protobuf:"bytes,15,opt,name=pipe_terminated,json=pipeTerminated,proto3,oneof"`
}

type ConnectResponse_PipePayloadRejected struct {
	PipePayloadRejected *PipePayloadRejected `protobuf:"bytes,16,opt,name=pipe_payload_rejected,json=pipePayloadRejected,proto3,oneof"`
}

type ConnectResponse_ListenerConfirmationAcknowledged struct {
	ListenerConfirmationAcknowledged *ListenerConfirmationAcknowledged `protobuf:"bytes,17,opt,name=listener_confirmation_acknowledged,json=listenerConfirmationAcknowledged,proto3,oneof"`
}

type ConnectResponse_ListenerBindFailed struct {
	ListenerBindFailed *ListenerBindFailed `protobuf:"bytes,18,opt,name=listener_bind_failed,json=listenerBindFailed,proto3,oneof"`
}

type ConnectResponse_ListenerUnbindFailed struct {
	ListenerUnbindFailed *ListenerUnbindFailed `protobuf:"bytes,19,opt,name=listener_unbind_failed,json=listenerUnbindFailed,proto3,oneof"`
}

type ConnectResponse_PipePayloadReceived struct {
	PipePayloadReceived *PipePayloadReceived `protobuf:"bytes,20,opt,name=pipe_payload_received,json=pipePayloadReceived,proto3,oneof"`
}

func (*ConnectResponse_ClientSessionOpened) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerBound) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerUnbound) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerOffer) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerEstablished) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerTerminated) isConnectResponse_Message() {}

func (*ConnectResponse_PipeOpened) isConnectResponse_Message() {}

func (*ConnectResponse_PipeOpenFailed) isConnectResponse_Message() {}

func (*ConnectResponse_PipeOpenUnknown) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerDecisionRejected) isConnectResponse_Message() {}

func (*ConnectResponse_OpenCancelAcknowledged) isConnectResponse_Message() {}

func (*ConnectResponse_PipeCloseAcknowledged) isConnectResponse_Message() {}

func (*ConnectResponse_OpenRequestRejected) isConnectResponse_Message() {}

func (*ConnectResponse_PipePayload) isConnectResponse_Message() {}

func (*ConnectResponse_PipeTerminated) isConnectResponse_Message() {}

func (*ConnectResponse_PipePayloadRejected) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerConfirmationAcknowledged) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerBindFailed) isConnectResponse_Message() {}

func (*ConnectResponse_ListenerUnbindFailed) isConnectResponse_Message() {}

func (*ConnectResponse_PipePayloadReceived) isConnectResponse_Message() {}

type Authenticate struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ClientId      string                 `protobuf:"bytes,1,opt,name=client_id,json=clientId,proto3" json:"client_id,omitempty"`
	ApiKeyId      string                 `protobuf:"bytes,2,opt,name=api_key_id,json=apiKeyId,proto3" json:"api_key_id,omitempty"`
	ApiKey        string                 `protobuf:"bytes,3,opt,name=api_key,json=apiKey,proto3" json:"api_key,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Authenticate) Reset() {
	*x = Authenticate{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Authenticate) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Authenticate) ProtoMessage() {}

func (x *Authenticate) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Authenticate.ProtoReflect.Descriptor instead.
func (*Authenticate) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{2}
}

func (x *Authenticate) GetClientId() string {
	if x != nil {
		return x.ClientId
	}
	return ""
}

func (x *Authenticate) GetApiKeyId() string {
	if x != nil {
		return x.ApiKeyId
	}
	return ""
}

func (x *Authenticate) GetApiKey() string {
	if x != nil {
		return x.ApiKey
	}
	return ""
}

type ClientSessionRef struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	ClientSessionId string                 `protobuf:"bytes,1,opt,name=client_session_id,json=clientSessionId,proto3" json:"client_session_id,omitempty"`
	ClientId        string                 `protobuf:"bytes,2,opt,name=client_id,json=clientId,proto3" json:"client_id,omitempty"`
	ApiKeyId        string                 `protobuf:"bytes,3,opt,name=api_key_id,json=apiKeyId,proto3" json:"api_key_id,omitempty"`
	AuthRevision    string                 `protobuf:"bytes,4,opt,name=auth_revision,json=authRevision,proto3" json:"auth_revision,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ClientSessionRef) Reset() {
	*x = ClientSessionRef{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClientSessionRef) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClientSessionRef) ProtoMessage() {}

func (x *ClientSessionRef) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClientSessionRef.ProtoReflect.Descriptor instead.
func (*ClientSessionRef) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{3}
}

func (x *ClientSessionRef) GetClientSessionId() string {
	if x != nil {
		return x.ClientSessionId
	}
	return ""
}

func (x *ClientSessionRef) GetClientId() string {
	if x != nil {
		return x.ClientId
	}
	return ""
}

func (x *ClientSessionRef) GetApiKeyId() string {
	if x != nil {
		return x.ApiKeyId
	}
	return ""
}

func (x *ClientSessionRef) GetAuthRevision() string {
	if x != nil {
		return x.AuthRevision
	}
	return ""
}

type ClientSessionOpened struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Session       *ClientSessionRef      `protobuf:"bytes,1,opt,name=session,proto3" json:"session,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ClientSessionOpened) Reset() {
	*x = ClientSessionOpened{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClientSessionOpened) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClientSessionOpened) ProtoMessage() {}

func (x *ClientSessionOpened) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClientSessionOpened.ProtoReflect.Descriptor instead.
func (*ClientSessionOpened) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{4}
}

func (x *ClientSessionOpened) GetSession() *ClientSessionRef {
	if x != nil {
		return x.Session
	}
	return nil
}

type BindListener struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	EndpointPattern string                 `protobuf:"bytes,1,opt,name=endpoint_pattern,json=endpointPattern,proto3" json:"endpoint_pattern,omitempty"`
	TargetId        string                 `protobuf:"bytes,2,opt,name=target_id,json=targetId,proto3" json:"target_id,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *BindListener) Reset() {
	*x = BindListener{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BindListener) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BindListener) ProtoMessage() {}

func (x *BindListener) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BindListener.ProtoReflect.Descriptor instead.
func (*BindListener) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{5}
}

func (x *BindListener) GetEndpointPattern() string {
	if x != nil {
		return x.EndpointPattern
	}
	return ""
}

func (x *BindListener) GetTargetId() string {
	if x != nil {
		return x.TargetId
	}
	return ""
}

type UnbindListener struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ListenerBindingId string                 `protobuf:"bytes,1,opt,name=listener_binding_id,json=listenerBindingId,proto3" json:"listener_binding_id,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *UnbindListener) Reset() {
	*x = UnbindListener{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UnbindListener) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UnbindListener) ProtoMessage() {}

func (x *UnbindListener) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UnbindListener.ProtoReflect.Descriptor instead.
func (*UnbindListener) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{6}
}

func (x *UnbindListener) GetListenerBindingId() string {
	if x != nil {
		return x.ListenerBindingId
	}
	return ""
}

type ListenerBinding struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ListenerBindingId string                 `protobuf:"bytes,1,opt,name=listener_binding_id,json=listenerBindingId,proto3" json:"listener_binding_id,omitempty"`
	EndpointPattern   string                 `protobuf:"bytes,2,opt,name=endpoint_pattern,json=endpointPattern,proto3" json:"endpoint_pattern,omitempty"`
	TargetId          string                 `protobuf:"bytes,3,opt,name=target_id,json=targetId,proto3" json:"target_id,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ListenerBinding) Reset() {
	*x = ListenerBinding{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerBinding) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerBinding) ProtoMessage() {}

func (x *ListenerBinding) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerBinding.ProtoReflect.Descriptor instead.
func (*ListenerBinding) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{7}
}

func (x *ListenerBinding) GetListenerBindingId() string {
	if x != nil {
		return x.ListenerBindingId
	}
	return ""
}

func (x *ListenerBinding) GetEndpointPattern() string {
	if x != nil {
		return x.EndpointPattern
	}
	return ""
}

func (x *ListenerBinding) GetTargetId() string {
	if x != nil {
		return x.TargetId
	}
	return ""
}

type ListenerBound struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Binding       *ListenerBinding       `protobuf:"bytes,1,opt,name=binding,proto3" json:"binding,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListenerBound) Reset() {
	*x = ListenerBound{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerBound) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerBound) ProtoMessage() {}

func (x *ListenerBound) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerBound.ProtoReflect.Descriptor instead.
func (*ListenerBound) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{8}
}

func (x *ListenerBound) GetBinding() *ListenerBinding {
	if x != nil {
		return x.Binding
	}
	return nil
}

type ListenerUnbound struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ListenerBindingId string                 `protobuf:"bytes,1,opt,name=listener_binding_id,json=listenerBindingId,proto3" json:"listener_binding_id,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ListenerUnbound) Reset() {
	*x = ListenerUnbound{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerUnbound) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerUnbound) ProtoMessage() {}

func (x *ListenerUnbound) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerUnbound.ProtoReflect.Descriptor instead.
func (*ListenerUnbound) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{9}
}

func (x *ListenerUnbound) GetListenerBindingId() string {
	if x != nil {
		return x.ListenerBindingId
	}
	return ""
}

type ListenerBindFailed struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	EndpointPattern string                 `protobuf:"bytes,1,opt,name=endpoint_pattern,json=endpointPattern,proto3" json:"endpoint_pattern,omitempty"`
	TargetId        string                 `protobuf:"bytes,2,opt,name=target_id,json=targetId,proto3" json:"target_id,omitempty"`
	Failure         ListenerBindingFailure `protobuf:"varint,3,opt,name=failure,proto3,enum=relaygate.relay.v1.ListenerBindingFailure" json:"failure,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ListenerBindFailed) Reset() {
	*x = ListenerBindFailed{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerBindFailed) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerBindFailed) ProtoMessage() {}

func (x *ListenerBindFailed) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerBindFailed.ProtoReflect.Descriptor instead.
func (*ListenerBindFailed) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{10}
}

func (x *ListenerBindFailed) GetEndpointPattern() string {
	if x != nil {
		return x.EndpointPattern
	}
	return ""
}

func (x *ListenerBindFailed) GetTargetId() string {
	if x != nil {
		return x.TargetId
	}
	return ""
}

func (x *ListenerBindFailed) GetFailure() ListenerBindingFailure {
	if x != nil {
		return x.Failure
	}
	return ListenerBindingFailure_LISTENER_BINDING_FAILURE_UNSPECIFIED
}

type ListenerUnbindFailed struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	ListenerBindingId string                 `protobuf:"bytes,1,opt,name=listener_binding_id,json=listenerBindingId,proto3" json:"listener_binding_id,omitempty"`
	Failure           ListenerBindingFailure `protobuf:"varint,2,opt,name=failure,proto3,enum=relaygate.relay.v1.ListenerBindingFailure" json:"failure,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ListenerUnbindFailed) Reset() {
	*x = ListenerUnbindFailed{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerUnbindFailed) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerUnbindFailed) ProtoMessage() {}

func (x *ListenerUnbindFailed) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerUnbindFailed.ProtoReflect.Descriptor instead.
func (*ListenerUnbindFailed) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{11}
}

func (x *ListenerUnbindFailed) GetListenerBindingId() string {
	if x != nil {
		return x.ListenerBindingId
	}
	return ""
}

func (x *ListenerUnbindFailed) GetFailure() ListenerBindingFailure {
	if x != nil {
		return x.Failure
	}
	return ListenerBindingFailure_LISTENER_BINDING_FAILURE_UNSPECIFIED
}

type Open struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Endpoint      string                 `protobuf:"bytes,2,opt,name=endpoint,proto3" json:"endpoint,omitempty"`
	TargetId      string                 `protobuf:"bytes,3,opt,name=target_id,json=targetId,proto3" json:"target_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Open) Reset() {
	*x = Open{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Open) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Open) ProtoMessage() {}

func (x *Open) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Open.ProtoReflect.Descriptor instead.
func (*Open) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{12}
}

func (x *Open) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *Open) GetEndpoint() string {
	if x != nil {
		return x.Endpoint
	}
	return ""
}

func (x *Open) GetTargetId() string {
	if x != nil {
		return x.TargetId
	}
	return ""
}

type CancelOpen struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CancelOpen) Reset() {
	*x = CancelOpen{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CancelOpen) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CancelOpen) ProtoMessage() {}

func (x *CancelOpen) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CancelOpen.ProtoReflect.Descriptor instead.
func (*CancelOpen) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{13}
}

func (x *CancelOpen) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

type ClosePipe struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PipeId        string                 `protobuf:"bytes,1,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ClosePipe) Reset() {
	*x = ClosePipe{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClosePipe) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClosePipe) ProtoMessage() {}

func (x *ClosePipe) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClosePipe.ProtoReflect.Descriptor instead.
func (*ClosePipe) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{14}
}

func (x *ClosePipe) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

type ListenerAccept struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AttemptId     string                 `protobuf:"bytes,1,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListenerAccept) Reset() {
	*x = ListenerAccept{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerAccept) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerAccept) ProtoMessage() {}

func (x *ListenerAccept) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerAccept.ProtoReflect.Descriptor instead.
func (*ListenerAccept) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{15}
}

func (x *ListenerAccept) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

type ListenerReject struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AttemptId     string                 `protobuf:"bytes,1,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListenerReject) Reset() {
	*x = ListenerReject{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerReject) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerReject) ProtoMessage() {}

func (x *ListenerReject) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerReject.ProtoReflect.Descriptor instead.
func (*ListenerReject) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{16}
}

func (x *ListenerReject) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

type ListenerConfirmed struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AttemptId     string                 `protobuf:"bytes,1,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	PipeId        string                 `protobuf:"bytes,2,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListenerConfirmed) Reset() {
	*x = ListenerConfirmed{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerConfirmed) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerConfirmed) ProtoMessage() {}

func (x *ListenerConfirmed) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerConfirmed.ProtoReflect.Descriptor instead.
func (*ListenerConfirmed) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{17}
}

func (x *ListenerConfirmed) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

func (x *ListenerConfirmed) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

type ListenerOffer struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	AttemptId         string                 `protobuf:"bytes,1,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	ListenerBindingId string                 `protobuf:"bytes,2,opt,name=listener_binding_id,json=listenerBindingId,proto3" json:"listener_binding_id,omitempty"`
	Endpoint          string                 `protobuf:"bytes,3,opt,name=endpoint,proto3" json:"endpoint,omitempty"`
	TargetId          string                 `protobuf:"bytes,4,opt,name=target_id,json=targetId,proto3" json:"target_id,omitempty"`
	CallerSessionId   string                 `protobuf:"bytes,5,opt,name=caller_session_id,json=callerSessionId,proto3" json:"caller_session_id,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ListenerOffer) Reset() {
	*x = ListenerOffer{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerOffer) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerOffer) ProtoMessage() {}

func (x *ListenerOffer) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerOffer.ProtoReflect.Descriptor instead.
func (*ListenerOffer) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{18}
}

func (x *ListenerOffer) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

func (x *ListenerOffer) GetListenerBindingId() string {
	if x != nil {
		return x.ListenerBindingId
	}
	return ""
}

func (x *ListenerOffer) GetEndpoint() string {
	if x != nil {
		return x.Endpoint
	}
	return ""
}

func (x *ListenerOffer) GetTargetId() string {
	if x != nil {
		return x.TargetId
	}
	return ""
}

func (x *ListenerOffer) GetCallerSessionId() string {
	if x != nil {
		return x.CallerSessionId
	}
	return ""
}

type ListenerEstablished struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AttemptId     string                 `protobuf:"bytes,1,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	PipeId        string                 `protobuf:"bytes,2,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListenerEstablished) Reset() {
	*x = ListenerEstablished{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerEstablished) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerEstablished) ProtoMessage() {}

func (x *ListenerEstablished) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerEstablished.ProtoReflect.Descriptor instead.
func (*ListenerEstablished) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{19}
}

func (x *ListenerEstablished) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

func (x *ListenerEstablished) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

type ListenerTerminated struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AttemptId     string                 `protobuf:"bytes,1,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	PipeId        string                 `protobuf:"bytes,2,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListenerTerminated) Reset() {
	*x = ListenerTerminated{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerTerminated) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerTerminated) ProtoMessage() {}

func (x *ListenerTerminated) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerTerminated.ProtoReflect.Descriptor instead.
func (*ListenerTerminated) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{20}
}

func (x *ListenerTerminated) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

func (x *ListenerTerminated) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

type PipeOpened struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	AttemptId     string                 `protobuf:"bytes,2,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	PipeId        string                 `protobuf:"bytes,3,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	Endpoint      string                 `protobuf:"bytes,4,opt,name=endpoint,proto3" json:"endpoint,omitempty"`
	TargetId      string                 `protobuf:"bytes,5,opt,name=target_id,json=targetId,proto3" json:"target_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PipeOpened) Reset() {
	*x = PipeOpened{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PipeOpened) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PipeOpened) ProtoMessage() {}

func (x *PipeOpened) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PipeOpened.ProtoReflect.Descriptor instead.
func (*PipeOpened) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{21}
}

func (x *PipeOpened) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *PipeOpened) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

func (x *PipeOpened) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

func (x *PipeOpened) GetEndpoint() string {
	if x != nil {
		return x.Endpoint
	}
	return ""
}

func (x *PipeOpened) GetTargetId() string {
	if x != nil {
		return x.TargetId
	}
	return ""
}

type PipeOpenFailed struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Endpoint      string                 `protobuf:"bytes,2,opt,name=endpoint,proto3" json:"endpoint,omitempty"`
	TargetId      string                 `protobuf:"bytes,3,opt,name=target_id,json=targetId,proto3" json:"target_id,omitempty"`
	Failure       OpenFailure            `protobuf:"varint,4,opt,name=failure,proto3,enum=relaygate.relay.v1.OpenFailure" json:"failure,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PipeOpenFailed) Reset() {
	*x = PipeOpenFailed{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PipeOpenFailed) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PipeOpenFailed) ProtoMessage() {}

func (x *PipeOpenFailed) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PipeOpenFailed.ProtoReflect.Descriptor instead.
func (*PipeOpenFailed) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{22}
}

func (x *PipeOpenFailed) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *PipeOpenFailed) GetEndpoint() string {
	if x != nil {
		return x.Endpoint
	}
	return ""
}

func (x *PipeOpenFailed) GetTargetId() string {
	if x != nil {
		return x.TargetId
	}
	return ""
}

func (x *PipeOpenFailed) GetFailure() OpenFailure {
	if x != nil {
		return x.Failure
	}
	return OpenFailure_OPEN_FAILURE_UNSPECIFIED
}

type PipeOpenUnknown struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Endpoint      string                 `protobuf:"bytes,2,opt,name=endpoint,proto3" json:"endpoint,omitempty"`
	TargetId      string                 `protobuf:"bytes,3,opt,name=target_id,json=targetId,proto3" json:"target_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PipeOpenUnknown) Reset() {
	*x = PipeOpenUnknown{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PipeOpenUnknown) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PipeOpenUnknown) ProtoMessage() {}

func (x *PipeOpenUnknown) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PipeOpenUnknown.ProtoReflect.Descriptor instead.
func (*PipeOpenUnknown) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{23}
}

func (x *PipeOpenUnknown) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *PipeOpenUnknown) GetEndpoint() string {
	if x != nil {
		return x.Endpoint
	}
	return ""
}

func (x *PipeOpenUnknown) GetTargetId() string {
	if x != nil {
		return x.TargetId
	}
	return ""
}

type OpenCancelAcknowledged struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	WasPending    bool                   `protobuf:"varint,2,opt,name=was_pending,json=wasPending,proto3" json:"was_pending,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OpenCancelAcknowledged) Reset() {
	*x = OpenCancelAcknowledged{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OpenCancelAcknowledged) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OpenCancelAcknowledged) ProtoMessage() {}

func (x *OpenCancelAcknowledged) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OpenCancelAcknowledged.ProtoReflect.Descriptor instead.
func (*OpenCancelAcknowledged) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{24}
}

func (x *OpenCancelAcknowledged) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *OpenCancelAcknowledged) GetWasPending() bool {
	if x != nil {
		return x.WasPending
	}
	return false
}

type PipeCloseAcknowledged struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PipeId        string                 `protobuf:"bytes,1,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	Owned         bool                   `protobuf:"varint,2,opt,name=owned,proto3" json:"owned,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PipeCloseAcknowledged) Reset() {
	*x = PipeCloseAcknowledged{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PipeCloseAcknowledged) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PipeCloseAcknowledged) ProtoMessage() {}

func (x *PipeCloseAcknowledged) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PipeCloseAcknowledged.ProtoReflect.Descriptor instead.
func (*PipeCloseAcknowledged) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{25}
}

func (x *PipeCloseAcknowledged) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

func (x *PipeCloseAcknowledged) GetOwned() bool {
	if x != nil {
		return x.Owned
	}
	return false
}

type OpenRequestRejected struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	RequestId     string                 `protobuf:"bytes,1,opt,name=request_id,json=requestId,proto3" json:"request_id,omitempty"`
	Failure       OpenRequestFailure     `protobuf:"varint,2,opt,name=failure,proto3,enum=relaygate.relay.v1.OpenRequestFailure" json:"failure,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OpenRequestRejected) Reset() {
	*x = OpenRequestRejected{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OpenRequestRejected) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OpenRequestRejected) ProtoMessage() {}

func (x *OpenRequestRejected) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OpenRequestRejected.ProtoReflect.Descriptor instead.
func (*OpenRequestRejected) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{26}
}

func (x *OpenRequestRejected) GetRequestId() string {
	if x != nil {
		return x.RequestId
	}
	return ""
}

func (x *OpenRequestRejected) GetFailure() OpenRequestFailure {
	if x != nil {
		return x.Failure
	}
	return OpenRequestFailure_OPEN_REQUEST_FAILURE_UNSPECIFIED
}

type ListenerDecisionRejected struct {
	state         protoimpl.MessageState  `protogen:"open.v1"`
	AttemptId     string                  `protobuf:"bytes,1,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	Failure       ListenerDecisionFailure `protobuf:"varint,2,opt,name=failure,proto3,enum=relaygate.relay.v1.ListenerDecisionFailure" json:"failure,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListenerDecisionRejected) Reset() {
	*x = ListenerDecisionRejected{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerDecisionRejected) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerDecisionRejected) ProtoMessage() {}

func (x *ListenerDecisionRejected) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerDecisionRejected.ProtoReflect.Descriptor instead.
func (*ListenerDecisionRejected) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{27}
}

func (x *ListenerDecisionRejected) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

func (x *ListenerDecisionRejected) GetFailure() ListenerDecisionFailure {
	if x != nil {
		return x.Failure
	}
	return ListenerDecisionFailure_LISTENER_DECISION_FAILURE_UNSPECIFIED
}

type PipePayload struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PipeId        string                 `protobuf:"bytes,1,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	Payload       []byte                 `protobuf:"bytes,2,opt,name=payload,proto3" json:"payload,omitempty"`
	PayloadId     string                 `protobuf:"bytes,3,opt,name=payload_id,json=payloadId,proto3" json:"payload_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PipePayload) Reset() {
	*x = PipePayload{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PipePayload) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PipePayload) ProtoMessage() {}

func (x *PipePayload) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PipePayload.ProtoReflect.Descriptor instead.
func (*PipePayload) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{28}
}

func (x *PipePayload) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

func (x *PipePayload) GetPayload() []byte {
	if x != nil {
		return x.Payload
	}
	return nil
}

func (x *PipePayload) GetPayloadId() string {
	if x != nil {
		return x.PayloadId
	}
	return ""
}

type PipePayloadReceived struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PipeId        string                 `protobuf:"bytes,1,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	PayloadId     string                 `protobuf:"bytes,2,opt,name=payload_id,json=payloadId,proto3" json:"payload_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PipePayloadReceived) Reset() {
	*x = PipePayloadReceived{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PipePayloadReceived) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PipePayloadReceived) ProtoMessage() {}

func (x *PipePayloadReceived) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PipePayloadReceived.ProtoReflect.Descriptor instead.
func (*PipePayloadReceived) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{29}
}

func (x *PipePayloadReceived) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

func (x *PipePayloadReceived) GetPayloadId() string {
	if x != nil {
		return x.PayloadId
	}
	return ""
}

type PipeTerminated struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PipeId        string                 `protobuf:"bytes,1,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PipeTerminated) Reset() {
	*x = PipeTerminated{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PipeTerminated) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PipeTerminated) ProtoMessage() {}

func (x *PipeTerminated) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PipeTerminated.ProtoReflect.Descriptor instead.
func (*PipeTerminated) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{30}
}

func (x *PipeTerminated) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

type PipePayloadRejected struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	PipeId        string                 `protobuf:"bytes,1,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	Failure       PipePayloadFailure     `protobuf:"varint,2,opt,name=failure,proto3,enum=relaygate.relay.v1.PipePayloadFailure" json:"failure,omitempty"`
	PayloadId     string                 `protobuf:"bytes,3,opt,name=payload_id,json=payloadId,proto3" json:"payload_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PipePayloadRejected) Reset() {
	*x = PipePayloadRejected{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PipePayloadRejected) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PipePayloadRejected) ProtoMessage() {}

func (x *PipePayloadRejected) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PipePayloadRejected.ProtoReflect.Descriptor instead.
func (*PipePayloadRejected) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{31}
}

func (x *PipePayloadRejected) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

func (x *PipePayloadRejected) GetFailure() PipePayloadFailure {
	if x != nil {
		return x.Failure
	}
	return PipePayloadFailure_PIPE_PAYLOAD_FAILURE_UNSPECIFIED
}

func (x *PipePayloadRejected) GetPayloadId() string {
	if x != nil {
		return x.PayloadId
	}
	return ""
}

type ListenerConfirmationAcknowledged struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	AttemptId     string                 `protobuf:"bytes,1,opt,name=attempt_id,json=attemptId,proto3" json:"attempt_id,omitempty"`
	PipeId        string                 `protobuf:"bytes,2,opt,name=pipe_id,json=pipeId,proto3" json:"pipe_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListenerConfirmationAcknowledged) Reset() {
	*x = ListenerConfirmationAcknowledged{}
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListenerConfirmationAcknowledged) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListenerConfirmationAcknowledged) ProtoMessage() {}

func (x *ListenerConfirmationAcknowledged) ProtoReflect() protoreflect.Message {
	mi := &file_proto_relaygate_relay_v1_relay_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListenerConfirmationAcknowledged.ProtoReflect.Descriptor instead.
func (*ListenerConfirmationAcknowledged) Descriptor() ([]byte, []int) {
	return file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP(), []int{32}
}

func (x *ListenerConfirmationAcknowledged) GetAttemptId() string {
	if x != nil {
		return x.AttemptId
	}
	return ""
}

func (x *ListenerConfirmationAcknowledged) GetPipeId() string {
	if x != nil {
		return x.PipeId
	}
	return ""
}

var File_proto_relaygate_relay_v1_relay_proto protoreflect.FileDescriptor

const file_proto_relaygate_relay_v1_relay_proto_rawDesc = "" +
	"\n" +
	"$proto/relaygate/relay/v1/relay.proto\x12\x12relaygate.relay.v1\"\xa8\a\n" +
	"\x0eConnectRequest\x12F\n" +
	"\fauthenticate\x18\x01 \x01(\v2 .relaygate.relay.v1.AuthenticateH\x00R\fauthenticate\x12G\n" +
	"\rbind_listener\x18\x02 \x01(\v2 .relaygate.relay.v1.BindListenerH\x00R\fbindListener\x12M\n" +
	"\x0funbind_listener\x18\x03 \x01(\v2\".relaygate.relay.v1.UnbindListenerH\x00R\x0eunbindListener\x12.\n" +
	"\x04open\x18\x04 \x01(\v2\x18.relaygate.relay.v1.OpenH\x00R\x04open\x12M\n" +
	"\x0flistener_accept\x18\x05 \x01(\v2\".relaygate.relay.v1.ListenerAcceptH\x00R\x0elistenerAccept\x12M\n" +
	"\x0flistener_reject\x18\x06 \x01(\v2\".relaygate.relay.v1.ListenerRejectH\x00R\x0elistenerReject\x12V\n" +
	"\x12listener_confirmed\x18\a \x01(\v2%.relaygate.relay.v1.ListenerConfirmedH\x00R\x11listenerConfirmed\x12A\n" +
	"\vcancel_open\x18\b \x01(\v2\x1e.relaygate.relay.v1.CancelOpenH\x00R\n" +
	"cancelOpen\x12>\n" +
	"\n" +
	"close_pipe\x18\t \x01(\v2\x1d.relaygate.relay.v1.ClosePipeH\x00R\tclosePipe\x12D\n" +
	"\fpipe_payload\x18\n" +
	" \x01(\v2\x1f.relaygate.relay.v1.PipePayloadH\x00R\vpipePayload\x12]\n" +
	"\x15pipe_payload_received\x18\v \x01(\v2'.relaygate.relay.v1.PipePayloadReceivedH\x00R\x13pipePayloadReceived\x12]\n" +
	"\x15pipe_payload_rejected\x18\f \x01(\v2'.relaygate.relay.v1.PipePayloadRejectedH\x00R\x13pipePayloadRejectedB\t\n" +
	"\amessage\"\xb6\x0e\n" +
	"\x0fConnectResponse\x12]\n" +
	"\x15client_session_opened\x18\x01 \x01(\v2'.relaygate.relay.v1.ClientSessionOpenedH\x00R\x13clientSessionOpened\x12J\n" +
	"\x0elistener_bound\x18\x02 \x01(\v2!.relaygate.relay.v1.ListenerBoundH\x00R\rlistenerBound\x12P\n" +
	"\x10listener_unbound\x18\x03 \x01(\v2#.relaygate.relay.v1.ListenerUnboundH\x00R\x0flistenerUnbound\x12J\n" +
	"\x0elistener_offer\x18\x04 \x01(\v2!.relaygate.relay.v1.ListenerOfferH\x00R\rlistenerOffer\x12\\\n" +
	"\x14listener_established\x18\x05 \x01(\v2'.relaygate.relay.v1.ListenerEstablishedH\x00R\x13listenerEstablished\x12Y\n" +
	"\x13listener_terminated\x18\x06 \x01(\v2&.relaygate.relay.v1.ListenerTerminatedH\x00R\x12listenerTerminated\x12A\n" +
	"\vpipe_opened\x18\a \x01(\v2\x1e.relaygate.relay.v1.PipeOpenedH\x00R\n" +
	"pipeOpened\x12N\n" +
	"\x10pipe_open_failed\x18\b \x01(\v2\".relaygate.relay.v1.PipeOpenFailedH\x00R\x0epipeOpenFailed\x12Q\n" +
	"\x11pipe_open_unknown\x18\t \x01(\v2#.relaygate.relay.v1.PipeOpenUnknownH\x00R\x0fpipeOpenUnknown\x12l\n" +
	"\x1alistener_decision_rejected\x18\n" +
	" \x01(\v2,.relaygate.relay.v1.ListenerDecisionRejectedH\x00R\x18listenerDecisionRejected\x12f\n" +
	"\x18open_cancel_acknowledged\x18\v \x01(\v2*.relaygate.relay.v1.OpenCancelAcknowledgedH\x00R\x16openCancelAcknowledged\x12c\n" +
	"\x17pipe_close_acknowledged\x18\f \x01(\v2).relaygate.relay.v1.PipeCloseAcknowledgedH\x00R\x15pipeCloseAcknowledged\x12]\n" +
	"\x15open_request_rejected\x18\r \x01(\v2'.relaygate.relay.v1.OpenRequestRejectedH\x00R\x13openRequestRejected\x12D\n" +
	"\fpipe_payload\x18\x0e \x01(\v2\x1f.relaygate.relay.v1.PipePayloadH\x00R\vpipePayload\x12M\n" +
	"\x0fpipe_terminated\x18\x0f \x01(\v2\".relaygate.relay.v1.PipeTerminatedH\x00R\x0epipeTerminated\x12]\n" +
	"\x15pipe_payload_rejected\x18\x10 \x01(\v2'.relaygate.relay.v1.PipePayloadRejectedH\x00R\x13pipePayloadRejected\x12\x84\x01\n" +
	"\"listener_confirmation_acknowledged\x18\x11 \x01(\v24.relaygate.relay.v1.ListenerConfirmationAcknowledgedH\x00R listenerConfirmationAcknowledged\x12Z\n" +
	"\x14listener_bind_failed\x18\x12 \x01(\v2&.relaygate.relay.v1.ListenerBindFailedH\x00R\x12listenerBindFailed\x12`\n" +
	"\x16listener_unbind_failed\x18\x13 \x01(\v2(.relaygate.relay.v1.ListenerUnbindFailedH\x00R\x14listenerUnbindFailed\x12]\n" +
	"\x15pipe_payload_received\x18\x14 \x01(\v2'.relaygate.relay.v1.PipePayloadReceivedH\x00R\x13pipePayloadReceivedB\t\n" +
	"\amessage\"b\n" +
	"\fAuthenticate\x12\x1b\n" +
	"\tclient_id\x18\x01 \x01(\tR\bclientId\x12\x1c\n" +
	"\n" +
	"api_key_id\x18\x02 \x01(\tR\bapiKeyId\x12\x17\n" +
	"\aapi_key\x18\x03 \x01(\tR\x06apiKey\"\x9e\x01\n" +
	"\x10ClientSessionRef\x12*\n" +
	"\x11client_session_id\x18\x01 \x01(\tR\x0fclientSessionId\x12\x1b\n" +
	"\tclient_id\x18\x02 \x01(\tR\bclientId\x12\x1c\n" +
	"\n" +
	"api_key_id\x18\x03 \x01(\tR\bapiKeyId\x12#\n" +
	"\rauth_revision\x18\x04 \x01(\tR\fauthRevision\"U\n" +
	"\x13ClientSessionOpened\x12>\n" +
	"\asession\x18\x01 \x01(\v2$.relaygate.relay.v1.ClientSessionRefR\asession\"V\n" +
	"\fBindListener\x12)\n" +
	"\x10endpoint_pattern\x18\x01 \x01(\tR\x0fendpointPattern\x12\x1b\n" +
	"\ttarget_id\x18\x02 \x01(\tR\btargetId\"@\n" +
	"\x0eUnbindListener\x12.\n" +
	"\x13listener_binding_id\x18\x01 \x01(\tR\x11listenerBindingId\"\x89\x01\n" +
	"\x0fListenerBinding\x12.\n" +
	"\x13listener_binding_id\x18\x01 \x01(\tR\x11listenerBindingId\x12)\n" +
	"\x10endpoint_pattern\x18\x02 \x01(\tR\x0fendpointPattern\x12\x1b\n" +
	"\ttarget_id\x18\x03 \x01(\tR\btargetId\"N\n" +
	"\rListenerBound\x12=\n" +
	"\abinding\x18\x01 \x01(\v2#.relaygate.relay.v1.ListenerBindingR\abinding\"A\n" +
	"\x0fListenerUnbound\x12.\n" +
	"\x13listener_binding_id\x18\x01 \x01(\tR\x11listenerBindingId\"\xa2\x01\n" +
	"\x12ListenerBindFailed\x12)\n" +
	"\x10endpoint_pattern\x18\x01 \x01(\tR\x0fendpointPattern\x12\x1b\n" +
	"\ttarget_id\x18\x02 \x01(\tR\btargetId\x12D\n" +
	"\afailure\x18\x03 \x01(\x0e2*.relaygate.relay.v1.ListenerBindingFailureR\afailure\"\x8c\x01\n" +
	"\x14ListenerUnbindFailed\x12.\n" +
	"\x13listener_binding_id\x18\x01 \x01(\tR\x11listenerBindingId\x12D\n" +
	"\afailure\x18\x02 \x01(\x0e2*.relaygate.relay.v1.ListenerBindingFailureR\afailure\"^\n" +
	"\x04Open\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x1a\n" +
	"\bendpoint\x18\x02 \x01(\tR\bendpoint\x12\x1b\n" +
	"\ttarget_id\x18\x03 \x01(\tR\btargetId\"+\n" +
	"\n" +
	"CancelOpen\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\"$\n" +
	"\tClosePipe\x12\x17\n" +
	"\apipe_id\x18\x01 \x01(\tR\x06pipeId\"/\n" +
	"\x0eListenerAccept\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x01 \x01(\tR\tattemptId\"/\n" +
	"\x0eListenerReject\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x01 \x01(\tR\tattemptId\"K\n" +
	"\x11ListenerConfirmed\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x01 \x01(\tR\tattemptId\x12\x17\n" +
	"\apipe_id\x18\x02 \x01(\tR\x06pipeId\"\xc3\x01\n" +
	"\rListenerOffer\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x01 \x01(\tR\tattemptId\x12.\n" +
	"\x13listener_binding_id\x18\x02 \x01(\tR\x11listenerBindingId\x12\x1a\n" +
	"\bendpoint\x18\x03 \x01(\tR\bendpoint\x12\x1b\n" +
	"\ttarget_id\x18\x04 \x01(\tR\btargetId\x12*\n" +
	"\x11caller_session_id\x18\x05 \x01(\tR\x0fcallerSessionId\"M\n" +
	"\x13ListenerEstablished\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x01 \x01(\tR\tattemptId\x12\x17\n" +
	"\apipe_id\x18\x02 \x01(\tR\x06pipeId\"L\n" +
	"\x12ListenerTerminated\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x01 \x01(\tR\tattemptId\x12\x17\n" +
	"\apipe_id\x18\x02 \x01(\tR\x06pipeId\"\x9c\x01\n" +
	"\n" +
	"PipeOpened\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x02 \x01(\tR\tattemptId\x12\x17\n" +
	"\apipe_id\x18\x03 \x01(\tR\x06pipeId\x12\x1a\n" +
	"\bendpoint\x18\x04 \x01(\tR\bendpoint\x12\x1b\n" +
	"\ttarget_id\x18\x05 \x01(\tR\btargetId\"\xa3\x01\n" +
	"\x0ePipeOpenFailed\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x1a\n" +
	"\bendpoint\x18\x02 \x01(\tR\bendpoint\x12\x1b\n" +
	"\ttarget_id\x18\x03 \x01(\tR\btargetId\x129\n" +
	"\afailure\x18\x04 \x01(\x0e2\x1f.relaygate.relay.v1.OpenFailureR\afailure\"i\n" +
	"\x0fPipeOpenUnknown\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x1a\n" +
	"\bendpoint\x18\x02 \x01(\tR\bendpoint\x12\x1b\n" +
	"\ttarget_id\x18\x03 \x01(\tR\btargetId\"X\n" +
	"\x16OpenCancelAcknowledged\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12\x1f\n" +
	"\vwas_pending\x18\x02 \x01(\bR\n" +
	"wasPending\"F\n" +
	"\x15PipeCloseAcknowledged\x12\x17\n" +
	"\apipe_id\x18\x01 \x01(\tR\x06pipeId\x12\x14\n" +
	"\x05owned\x18\x02 \x01(\bR\x05owned\"v\n" +
	"\x13OpenRequestRejected\x12\x1d\n" +
	"\n" +
	"request_id\x18\x01 \x01(\tR\trequestId\x12@\n" +
	"\afailure\x18\x02 \x01(\x0e2&.relaygate.relay.v1.OpenRequestFailureR\afailure\"\x80\x01\n" +
	"\x18ListenerDecisionRejected\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x01 \x01(\tR\tattemptId\x12E\n" +
	"\afailure\x18\x02 \x01(\x0e2+.relaygate.relay.v1.ListenerDecisionFailureR\afailure\"_\n" +
	"\vPipePayload\x12\x17\n" +
	"\apipe_id\x18\x01 \x01(\tR\x06pipeId\x12\x18\n" +
	"\apayload\x18\x02 \x01(\fR\apayload\x12\x1d\n" +
	"\n" +
	"payload_id\x18\x03 \x01(\tR\tpayloadId\"M\n" +
	"\x13PipePayloadReceived\x12\x17\n" +
	"\apipe_id\x18\x01 \x01(\tR\x06pipeId\x12\x1d\n" +
	"\n" +
	"payload_id\x18\x02 \x01(\tR\tpayloadId\")\n" +
	"\x0ePipeTerminated\x12\x17\n" +
	"\apipe_id\x18\x01 \x01(\tR\x06pipeId\"\x8f\x01\n" +
	"\x13PipePayloadRejected\x12\x17\n" +
	"\apipe_id\x18\x01 \x01(\tR\x06pipeId\x12@\n" +
	"\afailure\x18\x02 \x01(\x0e2&.relaygate.relay.v1.PipePayloadFailureR\afailure\x12\x1d\n" +
	"\n" +
	"payload_id\x18\x03 \x01(\tR\tpayloadId\"Z\n" +
	" ListenerConfirmationAcknowledged\x12\x1d\n" +
	"\n" +
	"attempt_id\x18\x01 \x01(\tR\tattemptId\x12\x17\n" +
	"\apipe_id\x18\x02 \x01(\tR\x06pipeId*\xf0\x01\n" +
	"\x16ListenerBindingFailure\x12(\n" +
	"$LISTENER_BINDING_FAILURE_UNSPECIFIED\x10\x00\x12,\n" +
	"(LISTENER_BINDING_FAILURE_INVALID_REQUEST\x10\x01\x12-\n" +
	")LISTENER_BINDING_FAILURE_CAPACITY_REACHED\x10\x02\x12%\n" +
	"!LISTENER_BINDING_FAILURE_CONFLICT\x10\x03\x12(\n" +
	"$LISTENER_BINDING_FAILURE_UNAVAILABLE\x10\x04*\x94\x02\n" +
	"\vOpenFailure\x12\x1c\n" +
	"\x18OPEN_FAILURE_UNSPECIFIED\x10\x00\x12 \n" +
	"\x1cOPEN_FAILURE_INVALID_REQUEST\x10\x01\x12 \n" +
	"\x1cOPEN_FAILURE_ROUTE_NOT_FOUND\x10\x02\x12\x1c\n" +
	"\x18OPEN_FAILURE_UNAVAILABLE\x10\x03\x12!\n" +
	"\x1dOPEN_FAILURE_CAPACITY_REACHED\x10\x04\x12\"\n" +
	"\x1eOPEN_FAILURE_LISTENER_REJECTED\x10\x05\x12\"\n" +
	"\x1eOPEN_FAILURE_DEADLINE_EXCEEDED\x10\x06\x12\x1a\n" +
	"\x16OPEN_FAILURE_CANCELLED\x10\a*h\n" +
	"\x12OpenRequestFailure\x12$\n" +
	" OPEN_REQUEST_FAILURE_UNSPECIFIED\x10\x00\x12,\n" +
	"(OPEN_REQUEST_FAILURE_DUPLICATE_IN_FLIGHT\x10\x01*\xd1\x01\n" +
	"\x17ListenerDecisionFailure\x12)\n" +
	"%LISTENER_DECISION_FAILURE_UNSPECIFIED\x10\x00\x12-\n" +
	")LISTENER_DECISION_FAILURE_INVALID_REQUEST\x10\x01\x121\n" +
	"-LISTENER_DECISION_FAILURE_ATTEMPT_NOT_PENDING\x10\x02\x12)\n" +
	"%LISTENER_DECISION_FAILURE_WRONG_PHASE\x10\x03*\xd5\x01\n" +
	"\x12PipePayloadFailure\x12$\n" +
	" PIPE_PAYLOAD_FAILURE_UNSPECIFIED\x10\x00\x12(\n" +
	"$PIPE_PAYLOAD_FAILURE_INVALID_REQUEST\x10\x01\x12\"\n" +
	"\x1ePIPE_PAYLOAD_FAILURE_NOT_OWNED\x10\x02\x12%\n" +
	"!PIPE_PAYLOAD_FAILURE_BACKPRESSURE\x10\x03\x12$\n" +
	" PIPE_PAYLOAD_FAILURE_UNAVAILABLE\x10\x042_\n" +
	"\x05Relay\x12V\n" +
	"\aConnect\x12\".relaygate.relay.v1.ConnectRequest\x1a#.relaygate.relay.v1.ConnectResponse(\x010\x01B?Z=github.com/cagojeiger/relaygate/internal/gen/relay/v1;relayv1b\x06proto3"

var (
	file_proto_relaygate_relay_v1_relay_proto_rawDescOnce sync.Once
	file_proto_relaygate_relay_v1_relay_proto_rawDescData []byte
)

func file_proto_relaygate_relay_v1_relay_proto_rawDescGZIP() []byte {
	file_proto_relaygate_relay_v1_relay_proto_rawDescOnce.Do(func() {
		file_proto_relaygate_relay_v1_relay_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_proto_relaygate_relay_v1_relay_proto_rawDesc), len(file_proto_relaygate_relay_v1_relay_proto_rawDesc)))
	})
	return file_proto_relaygate_relay_v1_relay_proto_rawDescData
}

var file_proto_relaygate_relay_v1_relay_proto_enumTypes = make([]protoimpl.EnumInfo, 5)
var file_proto_relaygate_relay_v1_relay_proto_msgTypes = make([]protoimpl.MessageInfo, 33)
var file_proto_relaygate_relay_v1_relay_proto_goTypes = []any{
	(ListenerBindingFailure)(0),              // 0: relaygate.relay.v1.ListenerBindingFailure
	(OpenFailure)(0),                         // 1: relaygate.relay.v1.OpenFailure
	(OpenRequestFailure)(0),                  // 2: relaygate.relay.v1.OpenRequestFailure
	(ListenerDecisionFailure)(0),             // 3: relaygate.relay.v1.ListenerDecisionFailure
	(PipePayloadFailure)(0),                  // 4: relaygate.relay.v1.PipePayloadFailure
	(*ConnectRequest)(nil),                   // 5: relaygate.relay.v1.ConnectRequest
	(*ConnectResponse)(nil),                  // 6: relaygate.relay.v1.ConnectResponse
	(*Authenticate)(nil),                     // 7: relaygate.relay.v1.Authenticate
	(*ClientSessionRef)(nil),                 // 8: relaygate.relay.v1.ClientSessionRef
	(*ClientSessionOpened)(nil),              // 9: relaygate.relay.v1.ClientSessionOpened
	(*BindListener)(nil),                     // 10: relaygate.relay.v1.BindListener
	(*UnbindListener)(nil),                   // 11: relaygate.relay.v1.UnbindListener
	(*ListenerBinding)(nil),                  // 12: relaygate.relay.v1.ListenerBinding
	(*ListenerBound)(nil),                    // 13: relaygate.relay.v1.ListenerBound
	(*ListenerUnbound)(nil),                  // 14: relaygate.relay.v1.ListenerUnbound
	(*ListenerBindFailed)(nil),               // 15: relaygate.relay.v1.ListenerBindFailed
	(*ListenerUnbindFailed)(nil),             // 16: relaygate.relay.v1.ListenerUnbindFailed
	(*Open)(nil),                             // 17: relaygate.relay.v1.Open
	(*CancelOpen)(nil),                       // 18: relaygate.relay.v1.CancelOpen
	(*ClosePipe)(nil),                        // 19: relaygate.relay.v1.ClosePipe
	(*ListenerAccept)(nil),                   // 20: relaygate.relay.v1.ListenerAccept
	(*ListenerReject)(nil),                   // 21: relaygate.relay.v1.ListenerReject
	(*ListenerConfirmed)(nil),                // 22: relaygate.relay.v1.ListenerConfirmed
	(*ListenerOffer)(nil),                    // 23: relaygate.relay.v1.ListenerOffer
	(*ListenerEstablished)(nil),              // 24: relaygate.relay.v1.ListenerEstablished
	(*ListenerTerminated)(nil),               // 25: relaygate.relay.v1.ListenerTerminated
	(*PipeOpened)(nil),                       // 26: relaygate.relay.v1.PipeOpened
	(*PipeOpenFailed)(nil),                   // 27: relaygate.relay.v1.PipeOpenFailed
	(*PipeOpenUnknown)(nil),                  // 28: relaygate.relay.v1.PipeOpenUnknown
	(*OpenCancelAcknowledged)(nil),           // 29: relaygate.relay.v1.OpenCancelAcknowledged
	(*PipeCloseAcknowledged)(nil),            // 30: relaygate.relay.v1.PipeCloseAcknowledged
	(*OpenRequestRejected)(nil),              // 31: relaygate.relay.v1.OpenRequestRejected
	(*ListenerDecisionRejected)(nil),         // 32: relaygate.relay.v1.ListenerDecisionRejected
	(*PipePayload)(nil),                      // 33: relaygate.relay.v1.PipePayload
	(*PipePayloadReceived)(nil),              // 34: relaygate.relay.v1.PipePayloadReceived
	(*PipeTerminated)(nil),                   // 35: relaygate.relay.v1.PipeTerminated
	(*PipePayloadRejected)(nil),              // 36: relaygate.relay.v1.PipePayloadRejected
	(*ListenerConfirmationAcknowledged)(nil), // 37: relaygate.relay.v1.ListenerConfirmationAcknowledged
}
var file_proto_relaygate_relay_v1_relay_proto_depIdxs = []int32{
	7,  // 0: relaygate.relay.v1.ConnectRequest.authenticate:type_name -> relaygate.relay.v1.Authenticate
	10, // 1: relaygate.relay.v1.ConnectRequest.bind_listener:type_name -> relaygate.relay.v1.BindListener
	11, // 2: relaygate.relay.v1.ConnectRequest.unbind_listener:type_name -> relaygate.relay.v1.UnbindListener
	17, // 3: relaygate.relay.v1.ConnectRequest.open:type_name -> relaygate.relay.v1.Open
	20, // 4: relaygate.relay.v1.ConnectRequest.listener_accept:type_name -> relaygate.relay.v1.ListenerAccept
	21, // 5: relaygate.relay.v1.ConnectRequest.listener_reject:type_name -> relaygate.relay.v1.ListenerReject
	22, // 6: relaygate.relay.v1.ConnectRequest.listener_confirmed:type_name -> relaygate.relay.v1.ListenerConfirmed
	18, // 7: relaygate.relay.v1.ConnectRequest.cancel_open:type_name -> relaygate.relay.v1.CancelOpen
	19, // 8: relaygate.relay.v1.ConnectRequest.close_pipe:type_name -> relaygate.relay.v1.ClosePipe
	33, // 9: relaygate.relay.v1.ConnectRequest.pipe_payload:type_name -> relaygate.relay.v1.PipePayload
	34, // 10: relaygate.relay.v1.ConnectRequest.pipe_payload_received:type_name -> relaygate.relay.v1.PipePayloadReceived
	36, // 11: relaygate.relay.v1.ConnectRequest.pipe_payload_rejected:type_name -> relaygate.relay.v1.PipePayloadRejected
	9,  // 12: relaygate.relay.v1.ConnectResponse.client_session_opened:type_name -> relaygate.relay.v1.ClientSessionOpened
	13, // 13: relaygate.relay.v1.ConnectResponse.listener_bound:type_name -> relaygate.relay.v1.ListenerBound
	14, // 14: relaygate.relay.v1.ConnectResponse.listener_unbound:type_name -> relaygate.relay.v1.ListenerUnbound
	23, // 15: relaygate.relay.v1.ConnectResponse.listener_offer:type_name -> relaygate.relay.v1.ListenerOffer
	24, // 16: relaygate.relay.v1.ConnectResponse.listener_established:type_name -> relaygate.relay.v1.ListenerEstablished
	25, // 17: relaygate.relay.v1.ConnectResponse.listener_terminated:type_name -> relaygate.relay.v1.ListenerTerminated
	26, // 18: relaygate.relay.v1.ConnectResponse.pipe_opened:type_name -> relaygate.relay.v1.PipeOpened
	27, // 19: relaygate.relay.v1.ConnectResponse.pipe_open_failed:type_name -> relaygate.relay.v1.PipeOpenFailed
	28, // 20: relaygate.relay.v1.ConnectResponse.pipe_open_unknown:type_name -> relaygate.relay.v1.PipeOpenUnknown
	32, // 21: relaygate.relay.v1.ConnectResponse.listener_decision_rejected:type_name -> relaygate.relay.v1.ListenerDecisionRejected
	29, // 22: relaygate.relay.v1.ConnectResponse.open_cancel_acknowledged:type_name -> relaygate.relay.v1.OpenCancelAcknowledged
	30, // 23: relaygate.relay.v1.ConnectResponse.pipe_close_acknowledged:type_name -> relaygate.relay.v1.PipeCloseAcknowledged
	31, // 24: relaygate.relay.v1.ConnectResponse.open_request_rejected:type_name -> relaygate.relay.v1.OpenRequestRejected
	33, // 25: relaygate.relay.v1.ConnectResponse.pipe_payload:type_name -> relaygate.relay.v1.PipePayload
	35, // 26: relaygate.relay.v1.ConnectResponse.pipe_terminated:type_name -> relaygate.relay.v1.PipeTerminated
	36, // 27: relaygate.relay.v1.ConnectResponse.pipe_payload_rejected:type_name -> relaygate.relay.v1.PipePayloadRejected
	37, // 28: relaygate.relay.v1.ConnectResponse.listener_confirmation_acknowledged:type_name -> relaygate.relay.v1.ListenerConfirmationAcknowledged
	15, // 29: relaygate.relay.v1.ConnectResponse.listener_bind_failed:type_name -> relaygate.relay.v1.ListenerBindFailed
	16, // 30: relaygate.relay.v1.ConnectResponse.listener_unbind_failed:type_name -> relaygate.relay.v1.ListenerUnbindFailed
	34, // 31: relaygate.relay.v1.ConnectResponse.pipe_payload_received:type_name -> relaygate.relay.v1.PipePayloadReceived
	8,  // 32: relaygate.relay.v1.ClientSessionOpened.session:type_name -> relaygate.relay.v1.ClientSessionRef
	12, // 33: relaygate.relay.v1.ListenerBound.binding:type_name -> relaygate.relay.v1.ListenerBinding
	0,  // 34: relaygate.relay.v1.ListenerBindFailed.failure:type_name -> relaygate.relay.v1.ListenerBindingFailure
	0,  // 35: relaygate.relay.v1.ListenerUnbindFailed.failure:type_name -> relaygate.relay.v1.ListenerBindingFailure
	1,  // 36: relaygate.relay.v1.PipeOpenFailed.failure:type_name -> relaygate.relay.v1.OpenFailure
	2,  // 37: relaygate.relay.v1.OpenRequestRejected.failure:type_name -> relaygate.relay.v1.OpenRequestFailure
	3,  // 38: relaygate.relay.v1.ListenerDecisionRejected.failure:type_name -> relaygate.relay.v1.ListenerDecisionFailure
	4,  // 39: relaygate.relay.v1.PipePayloadRejected.failure:type_name -> relaygate.relay.v1.PipePayloadFailure
	5,  // 40: relaygate.relay.v1.Relay.Connect:input_type -> relaygate.relay.v1.ConnectRequest
	6,  // 41: relaygate.relay.v1.Relay.Connect:output_type -> relaygate.relay.v1.ConnectResponse
	41, // [41:42] is the sub-list for method output_type
	40, // [40:41] is the sub-list for method input_type
	40, // [40:40] is the sub-list for extension type_name
	40, // [40:40] is the sub-list for extension extendee
	0,  // [0:40] is the sub-list for field type_name
}

func init() { file_proto_relaygate_relay_v1_relay_proto_init() }
func file_proto_relaygate_relay_v1_relay_proto_init() {
	if File_proto_relaygate_relay_v1_relay_proto != nil {
		return
	}
	file_proto_relaygate_relay_v1_relay_proto_msgTypes[0].OneofWrappers = []any{
		(*ConnectRequest_Authenticate)(nil),
		(*ConnectRequest_BindListener)(nil),
		(*ConnectRequest_UnbindListener)(nil),
		(*ConnectRequest_Open)(nil),
		(*ConnectRequest_ListenerAccept)(nil),
		(*ConnectRequest_ListenerReject)(nil),
		(*ConnectRequest_ListenerConfirmed)(nil),
		(*ConnectRequest_CancelOpen)(nil),
		(*ConnectRequest_ClosePipe)(nil),
		(*ConnectRequest_PipePayload)(nil),
		(*ConnectRequest_PipePayloadReceived)(nil),
		(*ConnectRequest_PipePayloadRejected)(nil),
	}
	file_proto_relaygate_relay_v1_relay_proto_msgTypes[1].OneofWrappers = []any{
		(*ConnectResponse_ClientSessionOpened)(nil),
		(*ConnectResponse_ListenerBound)(nil),
		(*ConnectResponse_ListenerUnbound)(nil),
		(*ConnectResponse_ListenerOffer)(nil),
		(*ConnectResponse_ListenerEstablished)(nil),
		(*ConnectResponse_ListenerTerminated)(nil),
		(*ConnectResponse_PipeOpened)(nil),
		(*ConnectResponse_PipeOpenFailed)(nil),
		(*ConnectResponse_PipeOpenUnknown)(nil),
		(*ConnectResponse_ListenerDecisionRejected)(nil),
		(*ConnectResponse_OpenCancelAcknowledged)(nil),
		(*ConnectResponse_PipeCloseAcknowledged)(nil),
		(*ConnectResponse_OpenRequestRejected)(nil),
		(*ConnectResponse_PipePayload)(nil),
		(*ConnectResponse_PipeTerminated)(nil),
		(*ConnectResponse_PipePayloadRejected)(nil),
		(*ConnectResponse_ListenerConfirmationAcknowledged)(nil),
		(*ConnectResponse_ListenerBindFailed)(nil),
		(*ConnectResponse_ListenerUnbindFailed)(nil),
		(*ConnectResponse_PipePayloadReceived)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_proto_relaygate_relay_v1_relay_proto_rawDesc), len(file_proto_relaygate_relay_v1_relay_proto_rawDesc)),
			NumEnums:      5,
			NumMessages:   33,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_proto_relaygate_relay_v1_relay_proto_goTypes,
		DependencyIndexes: file_proto_relaygate_relay_v1_relay_proto_depIdxs,
		EnumInfos:         file_proto_relaygate_relay_v1_relay_proto_enumTypes,
		MessageInfos:      file_proto_relaygate_relay_v1_relay_proto_msgTypes,
	}.Build()
	File_proto_relaygate_relay_v1_relay_proto = out.File
	file_proto_relaygate_relay_v1_relay_proto_goTypes = nil
	file_proto_relaygate_relay_v1_relay_proto_depIdxs = nil
}
