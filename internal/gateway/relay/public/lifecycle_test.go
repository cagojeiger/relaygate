...
func TestCrossOpeningStreamsDoNotHeadOfLineBlockListenerDecisions(t *testing.T) {
	...
	slots := make(map[string]routing.LiveBinding)
	bindings := &testBindingManager{bind: func(_ context.Context, session clientsession.Session, endpoint, targetID string, listener localbinding.ListenerEndpoint) (routing.LiveBinding, error) {
		...
		slot := routing.LiveBinding{
			Key: routing.BindingKey{ClientID: session.Ref.ClientID, EndpointPattern: endpoint, TargetID: targetID},
			Ref: ref,
		}
		mu.Lock()
